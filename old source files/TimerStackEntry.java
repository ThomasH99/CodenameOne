...
//            nextItem = sourceItemOrProject.getNextLeafItem(timedItem, condition); //now timedItem==entry.nextItem
            nextItem = sourceItemOrProject.getNextLeafItemN(timedItem); //now timedItem==entry.nextItem
            return;
        } else if (itemList != null) { //Timer is running on an ItemList
...
//                Item tempItem = ((Item) itemList.getItemAt(i)).getNextLeafItem(timedItem);
                Item tempItem = ((Item) itemList.get(i)).getNextLeafItemN(timedItem); //use get(), not getItemAt(), to only time filtered values
                if (tempItem != null) {
