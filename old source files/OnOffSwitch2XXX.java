 ...
 * with it. It has two types: Android and iOS. The types differ in the way that
 * they are rendered. The Android type (the default) is just a button with a
 * label that can be moved/dragged between the two states. The iOS version is
 * more elaborate due to the look of that platform.
 *
 * @author Shai Almog
 */
public class OnOffSwitch2XXX extends Container {
    ...
    private void initialize() {
        ...
        if (iosMode) {
            ...
            // Transform the switch mask so that we can get the proper background
            switchMaskImage = changeMaskToBGColor(switchMaskImage);
    ...
    // Function to change the mask to be the correct background color
    private Image changeMaskToBGColor(Image aImage) {
        int bgColor = getUnselectedStyle().getBgColor();

        int[] rgbBytes = aImage.getRGB();
        for (int iLoop = 0; iLoop < rgbBytes.length; iLoop++) {
            rgbBytes[iLoop] = (rgbBytes[iLoop] < 0) ? 0xff000000 | bgColor : rgbBytes[iLoop];
        }
        return Image.createImage(rgbBytes, aImage.getWidth(), aImage.getHeight());
    }
    ...
    public void paint(Graphics g) {
        if (iosMode) {
            ...
            if (dragged) {
                ...
                int offX;
                if (value) {
                    // Fixed to prevent the slider being dragged too far
                    int change = Math.min(deltaX, x + switchOnImage.getWidth() - 2 * switchButtonPadInt);
                    onX = x - change;
                    offX = x - change + switchOnImage.getWidth() - 2 * switchButtonPadInt;
                } else {
                    // Fixed to prevent the slider being dragged too far
                    int change = Math.max(deltaX, x - switchOnImage.getWidth() + 2 * switchButtonPadInt);
                    onX = x - change - switchOnImage.getWidth() + 2 * switchButtonPadInt;
                    offX = x - change;
                }
//                Log.p("onX: " + onX + ", offX: " + offX);
                switchButtonPadInt /= 2;
                ...
                g.setFont(s.getFont());
                g.setColor(0xffffff);
                // Shoudlnt be showing the text here if the option is off
                if (!noTextMode) {
