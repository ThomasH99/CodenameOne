...
/**
 * modified code from here: https://www.codenameone.com/blog/sticky-headers.html
 * by Chibuike Mba
 *
 * @author Chen.
 */
public class StickyHeader2 extends Container implements ScrollListener {
...
//            hideFollowingComponents(hidden);
                ...
                for (int i = index + 1; i < count; i++) {
                    ...
                    if (comp instanceof StickyHeader2) {
                        break;
                    } else {
        ...
        button.addLongPressListener((e) -> {
            ...
            if (hidden) {
                for (Component comp : getParent().getChildrenAsList(true)) {
                    if (comp instanceof StickyHeader2) {
                        ((StickyHeader2) comp).hidden = false;
//                        ((StickyHeader) comp).hideShowButton.setMaterialIcon(Icons.iconCollapseListStickyHeader);
                        ((StickyHeader2) comp).button.setMaterialIcon(Icons.iconCollapseListStickyHeader);
                    } else {
                ...
                for (Component comp : getParent().getChildrenAsList(true)) {
                    if (comp instanceof StickyHeader2) {
                        ((StickyHeader2) comp).hidden = true;
//                        ((StickyHeader) comp).hideShowButton.setMaterialIcon(Icons.iconExpandListStickyHeader);
                        ((StickyHeader2) comp).button.setMaterialIcon(Icons.iconExpandListStickyHeader);
                    } else {
                        comp.setHidden(true);
                    }
                }
...
//            e.consume(); //prevent event to be send to component *below* stickyHeader (below glasspane)? Not working
    ...
    public StickyHeader2(String uiid) {
        this();
//        this.setUIID(uiid);
        button.setUIID(uiid);
    }
...
//    }
    ...
    private void popFromHeader() {
        ArrayList stack = (ArrayList) getParent().getClientProperty("sticky");
        StickyHeader2 h1 = (StickyHeader2) stack.remove(0); //remove current stickyHeader
        h1.comp.remove();
        h1.addComponent(h1.comp); //add orginal content back into this stickyHeader

        if (!stack.isEmpty()) {
            StickyHeader2 h = (StickyHeader2) stack.get(0);
...
//            getComponentForm().getLayeredPane().removeComponent(comp); //remove stickyHeader content from North pane
    ...
    private void pushToHeader() {
        ...
        if (!stack.isEmpty()) {
            StickyHeader2 h = (StickyHeader2) stack.get(0); //get last pushed stickyHeader
            if (getY() < h.getY()) { //if last pushed is further down the screen, no need to do anything for this stickyHeader
                return;
            } else h.comp.remove(); //remove previous stickyHeader's content from stickyHeader content holder
        } //else
