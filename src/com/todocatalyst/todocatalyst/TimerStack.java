...
//        GetNextItemStatus nextTimedItem = timerInstance.updateToNextTimerItem(true, true); //get next timed item if any, also update project and save
...
//</editor-fold>
    ...
    private Item goToNextTimedItemImpl(boolean update) {
        ...
        while (nextTimedItem == null && currentTimerStackIndex >= 0) { //if no more tasks in current timer, try earlier ones
            TimerInstance timerInstance = activeTimers.get(currentTimerStackIndex);
//            GetNextItemStatus nextTimedItemStatus = timerInstance.updateToNextTimerItem(update, update); //DO NOT update since this is a previous Timer where the current item was interr8pted, so simply continue with that
            GetNextItemStatus nextTimedItemStatus = timerInstance.updateToNextTimerItem(update); //DO NOT update since this is a previous Timer where the current item was interr8pted, so simply continue with that
...
//        item.addElapsedTimerTimeToActual( elapsedTime); //false
     ...
     * @return
     */
//    public boolean isTimerTimingThisNowXXX(Item timedItem) {
...
//    }
     ...
     * @return
     */
//    public boolean isATimerActiveForXXX(ItemAndListCommonInterface timedItemOrProjectOrItemList) {
...
//                    || timedItemOrProjectOrItemList == timerInstance.getTimedItemListN()) {
//                return true;
//            }
//            if (Config.TEST) {
...
//        return false;
//    }
//    public boolean isATimerActiveForThisProjectXXX(Item project) {
//        if (project != null) {
//            return false;
//        }
//        for (TimerInstance timerInstance : activeTimers) {
...
//            if ((itemList != null && itemList == timerInstance.getTimedItemListN())) {
...
//                    MyForm.showToastBar("Already running Timer paused for \"" + previousTimerInstance.getTimedItemN().getText() + "\", will continue after this "
    ...
    private void startTimer(Item timedItemOrProject, ItemList timedItemList, MyForm previousForm,
...
//        this.timeEvenInvalidItem = timeEvenInvalidItem;
        ASSERT.that(!interruptOrInstantTask || timedItemList == null, "cannot have an interrupt task with a list");
        ASSERT.that(timedItemList != null || timedItemOrProject != null, "cannot launch timer if both list and item are null");
//<editor-fold defaultstate="collapsed" desc="comment">
//        if (false && isItemOrListAlreadyBeingTimed(timedItemOrProject, timedItemList)) { //don't allow to start timer again on an element which is already timer (and could be interrupted by later timers)
//            return;
//        }
//        TimerInstance timerInstance = getCurrentTimerInstanceN();
//        if (timerInstance != null) { //timer already active
//</editor-fold>
        ...
        if (!isTimerActive() //timer not already running
                || ((timedItemList != null || timedItemOrProject != null)
                && (((timedItemList == null || getCurrentTimerInstanceN().getTimedItemListN() != timedItemList) //and not already running on this list/item
                && (timedItemOrProject == null || getCurrentTimerInstanceN().getTimedItemN() != timedItemOrProject))) // or item
                ...
                || showDialogPauseActiveTimer(timedItemList != null ? timedItemList : timedItemOrProject))))) { //or setting to ask before interrupting
//<editor-fold defaultstate="collapsed" desc="comment">
//starting up timer (interrupt already running timer)
//                    if (timedItemList instanceof ItemList) {
//                        TimerStack.getInstance().startTimerOnItemList((ItemList) timedItemList, previousForm); //itemListOrg because Timer stores the original Parse objects and does its own filter/sort
//                    } else if (timedItemOrProject instanceof Item) {
//                        TimerStack.getInstance().startTimerOnItem((Item) timedItemOrProject, previousForm); //itemListOrg because Timer stores the original Parse objects and does its own filter/sort
//                    }
//</editor-fold>
            ...
            if (newTimedItem != null) { //if there's an item to time in the new list/item/project
                ...
                //no save of new TimerInstance if no tasks to time
...
//<editor-fold defaultstate="collapsed" desc="comment">
//            getCurrentTimerInstanceN().setFullScreen(true);
//            getCurrentTimerInstanceN().saveMe();
//            new ScreenTimer6(previousForm).show(); //should show BigTimer (unless smallTimer is shown? NO, always show big timer since starting timer first time is handled above)
//</editor-fold>
            if (timedItemList != null && getCurrentTimerInstanceN().getTimedItemListN() == timedItemList) {
                MyForm.showToastBar("Timer already running on \"" + timedItemList.getText() + "\"");
            }
            refreshOrShowUIOnTimerChange();
        }
//<editor-fold defaultstate="collapsed" desc="comment">
...
//    }
    public boolean stopTimerIfActiveOnThisItemAndGotoNext(Item item, boolean saveUpdatedItem, boolean saveUpdatedTimerInstance, boolean gotoNextItem) { //boolean continueWithNextItem // 
...
//</editor-fold>
    ...
    public void updateTimerWhenItemListIsDeleted(ItemList itemList) {
        for (TimerInstance timerInstance : activeTimers) {
            if (itemList == timerInstance.getTimedItemListN()) {
                timerInstance.stopTimerOnTimedItemAndUpdateActualsAndSave(true); //update the actuals of timed item
                activeTimers.remove(timerInstance);
                refreshOrShowUIOnTimerChange(); //refresh wheter deleted or modified
                return;
            }
        }
    }
...
//    public void updateTimerOnItemStatusChange(ItemStatus oldStatus, ItemStatus newStatus, boolean startTimingNextItem, boolean saveItem) {
    public void updateTimerOnItemStatusChange(Item item, boolean startTimingNextItem, boolean saveItem) {
        Item timedItemN = getCurrentlyTimedItemN();
        if (timedItemN == item) { // && newStatus != oldStatus) {
...
//            TimerInstance timerInstance = TimerStack.getInstance().activeTimers.remove(TimerStack.getInstance().activeTimers.size() - 1); //pop last timerInstance since it has no more tasks
            TimerInstance timerInstance = activeTimers.remove(activeTimers.size() - 1); //pop last timerInstance since it has no more tasks
            timerInstance.stopTimerOnTimedItemAndUpdateActualsAndSave(false); //no save since deleted below
            timerInstance.deleteInstance();
        }
...
//                        }
    ...
    private void updateCurrentlyTimedItemWithEditedElapsedTime(long elapsedTime) {
        ...
        if (timerInstance2 != null) {
            Item timedItem = timerInstance2.getTimedItemN();
...
//    protected static Container buildContentPaneSmall(Form form, Container contentPane) {
    protected static Container buildContentPaneSmallN(MyForm myForm) {
//        return buildContentPane(form, contentPane, false, null);
        return buildContentPaneN(myForm, false, null);
    }

    static boolean longPressed = false; //prevent that a longPress also triggers a normalPress on the Timer button
...
//    private static Container buildContentPane(Form form, Container contentPane, boolean fullScreenTimer, SaveEditedValuesLocally formPreviousValues) {
    private static Container buildContentPaneN(MyForm myForm, boolean fullScreenTimer, SaveEditedValuesLocally formPreviousValues) {
        longPressed = false; //always reset when rebuilding a new timer
...
//        assert currEntry != null : "entry must always be defined";
        TimerStack model = TimerStack.getInstance();
        TimerInstance timerInstance = model.getCurrentTimerInstanceN();
        if (timerInstance == null) {
            return null;
        }
...
//        Item timedItem = timerInstance.getTimedItemN(); //currEntry.timedItem;
        ...
        ItemList itemList = timerInstance.getTimedItemListN();
...
//</editor-fold>
//        MyButtonLongPress timerStartStopButton = new MyButtonLongPress();
        Button timerStartStopButton = new Button();
...
//        Map<Object, MyForm.UpdateField> parseIdMap2 = new HashMap<Object, MyForm.UpdateField>(); //create a new hashmap for this item
        ParseIdMap2 parseIdMap2 = new ParseIdMap2(); //create a new hashmap for this item
...
//        Container timerBigTimerTopLevelContainer = new Container(BorderLayout.center()); //top-level container for TimerScreen
...
//        Container timerPickerContainer = new Container();
//        
//        timerContent.add(BorderLayout.CENTER,timerContainer);
//        Container timerButtonsContainer = new Container(BoxLayout.y());
//        timerContent.add(BorderLayout.SOUTH,timerButtonsContainer);
//<editor-fold defaultstate="collapsed" desc="comment">
//        if (false) {
...
////<editor-fold defaultstate="collapsed" desc="comment">
//////                TimerInstance timerInstance2 = TimerStack.getInstance().getCurrentTimerInstanceN();
////                TimerInstance timerInstance2 = model.getCurrentTimerInstanceN();
////                //UI: OK to longPress a running timer (no need to stop it, will simply adjust timer on Done
////                if (timerInstance2 != null) {
////
////                    long oldElapsedTime = timerInstance2.getElapsedTime();
...
////                }
////</editor-fold>
//                    model.updateCurrentlyTimedItemWithEditedElapsedTime(hiddenElapsedTimePicker.getDuration());
//
...
//        }
//</editor-fold>
//         boolean longPressed = false; //prevent that a longPress also triggers a normalPress on the Timer button
        timerStartStopButton.addLongPressListener(ev -> {
            longPressed = true;
//            ev.consume();

            hiddenElapsedTimePicker.setDuration(timerInstance.getElapsedTimeToDisplay()); //must edit total elapsed time since that is what is shown
//            hiddenElapsedTimePicker.remove(); //always remove just in case
//            timerPickerContainer.add(hiddenElapsedTimePicker); //only add when actually needed to avoid activation when moving from other fields
            ...
            hiddenElapsedTimePicker.addActionListener((e) -> {
                ...
                hiddenElapsedTimePicker.stopEditing(null);
                refreshElapsedTime.actionPerformed(null);
                hiddenElapsedTimePicker.remove(); //remove again when done
                refreshTotalActualEffort.actionPerformed(null);
            });
...
//</editor-fold>
            ...
            if (status.getStatus() == ItemStatus.CREATED //DON'T revert eg cancelled/done/waiting task to ongoing just because time is spent on it
                ...
                //isDirty, since on slow network/parse server, the item may not have been saved yet
                ASSERT.that(!timedItem.isDirty() || timedItem.getStatus() == ItemStatus.CREATED,
...
//                DAO.getInstance().saveInBackground(timedItem); //ongoing value
                long timerStartedOn = MyDate.currentTimeMillis() - MyPrefs.timerMinimumTimeRequiredToSetTaskOngoingAndToUpdateActualsInSeconds.getInt() * MyDate.SECOND_IN_MILLISECONDS;
//                TimerStack.updateTimedItemToOngoing(timedItem, new MyDate(timerStartedOn));
                TimerStack.updateTimedItemToOngoing(status, timedItem);//, new MyDate(timerStartedOn));
                //update 
...
//        ActionListener returnScreenSaverToPreviousStateXXX //restore screenSaver when exiting BigTimer or ScreenTimer6 (exiting to SmallTimer)
//                = (e) -> {
...
//                };
        ...
        if (Display.getInstance().isScreenSaverDisableSupported()
                //                && !MyPrefs.keepScreenAlwaysOnInApp.getBoolean()
                && MyPrefs.timerKeepScreenAlwaysOnInTimer.getBoolean()) {
            //TNB. previous state is defined by keepScreenAlwaysOnInApp
...
//                model.updateTimedTaskSetStatusAndGotoNext(null);
                model.moveToNextTask();
...
//                if (false&&current instanceof ScreenTimer6) { //done in refreshOrShowUIOnTimerChange()
//                    ((ScreenTimer6) current).backCommand.actionPerformed(null);
//                }
//                returnScreenSaverToPreviousState.actionPerformed(null); //NOW done in backCmd
                if (false) {
...
//                refreshScreenOnTimerUpdate();
//</editor-fold>
//                getInstance().gotoNext(timerInstance, timedItem, ItemStatus.DONE);
//                model.gotoNext(timerInstance, timedItem, ItemStatus.DONE);
//                model.updateTimedTaskSetStatusAndGotoNext(ItemStatus.DONE);
                Item timedItem = getInstance().getCurrentlyTimedItemN(); //updating the timed item will automatically update the Timer
                if (timedItem != null) {
                    timedItem.setStatus(ItemStatus.DONE); //updating the timed item will automatically update the Timer
...
//                    DAO.getInstance().saveNewTriggerUpdate();
                    DAO.getInstance().saveToParseNow(getInstance().getCurrentlyTimedItemN());
                    getInstance().refreshOrShowUIOnTimerChange();
                }
            }
        };
...
//                TimerStack.getInstance().refreshOrShowTimerUI();
//                refreshScreenOnTimerUpdate();
//                getInstance().gotoNext(timerInstance, timedItem, ItemStatus.WAITING);
...
//        Item nextComingItem = TimerStack.getInstance().getNextComingItem();
        Item nextComingItem = model.getNextComingItem();
...
//            cmdGotoFullScreenTimer = CommandTracked.create("", Icons.iconEditSymbolLabelStyle, (e) -> {
            if (false) {
                cmdGotoFullScreenTimer = CommandTracked.create("", Icons.iconEdit, (e) -> {
//                @Override
//                public void actionPerformed(ActionEvent evt) {
...
//        if (fullScreenTimer) {
//            contentPane = new Container(BoxLayout.y());
//        } else {
//            MyBorderLayout myBorderLayout = new MyBorderLayout(MyBorderLayout.CENTER_BEHAVIOR_CENTER);
////            myBorderLayout.setSizeEastWestMode(MyBorderLayout.SIZE_WEST_BEFORE_EAST);
//            myBorderLayout.setSizeEastWestMode(MyBorderLayout.SIZE_EAST_BEFORE_WEST); //first size East to ensure timer + (>) are visible
//            contentPane = new Container(myBorderLayout);
//        }
//        Container contentPane = fullScreenTimer?new Container(BoxLayout.y()):new Container(new MyBorderLayout(MyBorderLayout.CENTER_BEHAVIOR_CENTER,MyBorderLayout.SIZE_EAST_BEFORE_WEST));
//        Container contentPane = fullScreenTimer ? new Container(BoxLayout.y()) : new Container(new MyBorderLayout(MyBorderLayout.SIZE_WEST_BEFORE_EAST));
...
//                    timerContainer.add(hierarchyCont);
//                }
//            }
//
//        if (fullScreenTimer) {
...
//            description.setUIID("Label");
            ...
            description.setText(timedItem.getText());
            description.setTraversable(false); //avoid that tapping Next in a PinchInsert container will activate this field
            if (timedItem.isInteruptOrInstantTask() && description.getText().equals("")) {
//                    contentPane.getComponentForm().setEditOnShow(description); //UI: for interrupt/instant tasks or new tasks (no previous text), automatically enter into description field 
//                myForm.setEditOnShow(description); //UI: for interrupt/instant tasks or new tasks (no previous text), automatically enter into description field 
...
//        editItemButton = new Button(MyReplayCommand.create("EditItemFromTimer-", timedItem.getObjectIdP(), "", Icons.iconEditSymbolLabelStyle,
        editItemButton = new Button(MyReplayCommand.create("EditItemFromTimer-", timedItem.getReplayId(), "", Icons.iconEdit,
                (e) -> {
...
//<editor-fold defaultstate="collapsed" desc="comment">
                        if (false) { //SINCE newActual is the item's edited Actual, not updated with elapsed time before editing the item, it should not be possible to become less than
                            ...
                            long newActual = timedItem.getActualForTaskItself();
...
//                            timerInst.setElapsedTime(newActual);
                                ...
                                timedItem.setActualForTaskItself(0, false);
                                timerInst.saveMe();
                                MyForm.showToastBar("The task's worked time edited to LESS");
                            } else {
                                //no change to elapsed time
                                timedItem.setActualForTaskItself(newActual - timerInst.getElapsedTime(), false); //update item.actual to edited value minus the elapsed time
                            }
                        }
//</editor-fold>
...
//        if (fullScreenTimer) {
//            timerContainer.add(BorderLayout.west(status).add(BorderLayout.CENTER, description).add(BorderLayout.EAST, editItemButton));
//        }
...
//            Container timerContainer = fullScreenTimer ? new Container(BoxLayout.y()) : new Container(new MyBorderLayout(BorderLayout.CENTER_BEHAVIOR_CENTER, MyBorderLayout.SIZE_EAST_BEFORE_WEST));
            ...
            timerContainer.setScrollableY(true);
            ...
            Container timerPickerContainer = new Container();
            //            timerContainer.add(timerPickerContainer);
            timerButtonsContainer.add(timerPickerContainer);
//            timerContainer.add(BorderLayout.SOUTH, timerPickerContainer);

            timerPickerContainer.add(hiddenElapsedTimePicker); //only add when actually needed to avoid activation when moving from other fields

            timerBigTimerTopLevelContainer.add(BorderLayout.CENTER, timerContainer);
...
//                    timedItem.setEstimate(remainingEffort.getDuration(), false);
                    timedItem.setEstimateForTask(effort.getDuration(), false); //saved immediately on edit
...
//            );
            ...
            timerContainer.add(effortDetailsCont);

            estimateTable.add(tl.createConstraint().widthPercentage(33).horizontalAlign(Component.CENTER), new Label(Item.EFFORT_ESTIMATE_SHORT, "BigTimerEffortLabel")); //"Estimate")); //leftalign labels (like the Tickers)
            estimateTable.add(tl.createConstraint().widthPercentage(34).horizontalAlign(Component.CENTER), new Label(Item.EFFORT_TOTAL_SHORT, "BigTimerEffortLabel")); //"Total"));
            estimateTable.add(tl.createConstraint().widthPercentage(33).horizontalAlign(Component.CENTER), new Label(Item.EFFORT_REMAINING_SHORT, "BigTimerEffortLabel")); //"Remaining"));
            ...
            timerContainer.add(BorderLayout.center(estimateTable));
...
//        if (currEntry.interruptOrInstantTask) {
            ...
            if (false) {
                timerContainer.add(hiddenElapsedTimePicker);
            }
            timerContainer.setScrollableY(true); //since the size of the timer may overflow
...
//                timerContainer.add(GridLayout.encloseIn(1, completedButton));
                timerButtonsContainer.add(GridLayout.encloseIn(1, completedButton));
            }

            if (setWaitingButton != null && nextButton != null) {
//                timerContainer.add(GridLayout.encloseIn(3, exitButton, setWaitingButton, nextButton));
                timerButtonsContainer.add(GridLayout.encloseIn(3, exitButton, setWaitingButton, nextButton));
            } else if (setWaitingButton != null) {
//                timerContainer.add(GridLayout.encloseIn(2, exitButton, setWaitingButton));
                timerButtonsContainer.add(GridLayout.encloseIn(2, exitButton, setWaitingButton));
            } else if (nextButton != null) {
//                timerContainer.add(GridLayout.encloseIn(2, exitButton, nextButton));
                timerButtonsContainer.add(GridLayout.encloseIn(2, exitButton, nextButton));
            } else {
//                timerContainer.add(GridLayout.encloseIn(1, exitButton));
                timerButtonsContainer.add(GridLayout.encloseIn(1, exitButton));
            }
//                        nextTaskCont.add(gotoNextTaskButtonWithItemText);
            if (gotoNextTaskButtonWithItemText != null) {
//                timerContainer.add(GridLayout.encloseIn(1, gotoNextTaskButtonWithItemText));
...
//</editor-fold>
//            return timerContainer;
            return timerBigTimerTopLevelContainer;
        } else { //SMALL TIMER container
//            Container timerContainer = fullScreenTimer ? new Container(BoxLayout.y()) : new Container(new MyBorderLayout(BorderLayout.CENTER_BEHAVIOR_CENTER, MyBorderLayout.SIZE_EAST_BEFORE_WEST));
            Container timerContainer = new Container(new MyBorderLayout(BorderLayout.CENTER_BEHAVIOR_CENTER, MyBorderLayout.SIZE_EAST_BEFORE_WEST));
            timerContainer.add(BorderLayout.SOUTH, hiddenElapsedTimePicker); //only add when actually needed to avoid activation when moving from other fields
...
//                Container swipeSmallContainer = new SwipeableContainer(nextTask, null, contentPane);
            ...
            nextTask.setText("");
            timerContainer.setUIID("SmallTimerContainer");
//                Container swipeable = new SwipeableContainer(BoxLayout.encloseX(nextTask,exitTimer), null, contentPane);
            SwipeableContainer swipeable = new SwipeableContainer(null, BoxLayout.encloseX(exitTimer, nextTask), timerContainer) {
...
//            timerContainer.add(BorderLayout.WEST, west);
//            Component text;
...
////                    text = new SpanLabel(timedItem.getText() + "->" + (nextComingItem != null ? nextComingItem.getText() : "<>")); //otherwise just show as label
//                    description.setText(timedItem.getText() + "->" + (nextComingItem != null ? nextComingItem.getText() : "<>")); //otherwise just show as label
//                } else {
////                    west.add(new Label(timedItem.getText())); //otherwise just show as label
////                    text = new Label(timedItem.getText()); //otherwise just show as label
//                    description.setText(timedItem.getText()); //otherwise just show as label
//                }
//            }
//</editor-fold>
            description.setText(timedItem.getText()); //otherwise just show as label
...
//            west.add(text);
//            west.add(description);
            timerContainer.add(BorderLayout.CENTER, BoxLayout.encloseX(description));
            timerContainer.add(BorderLayout.WEST, status);

            /*East*/
            Container east = BorderLayout.centerAbsolute(BoxLayout.encloseXNoGrow(timerStartStopButton));
            east.add(BorderLayout.EAST, fullScreenTimerButton);
            timerContainer.add(BorderLayout.EAST,
                    //                    BoxLayout.encloseXNoGrow(timerStartStopButton, fullScreenTimerButton));
                    //                    BorderLayout.center(BoxLayout.encloseXNoGrow(timerStartStopButton, fullScreenTimerButton)));
                    //                    BoxLayout.center(BoxLayout.encloseXNoGrow(timerStartStopButton, fullScreenTimerButton)));
                    east);
//            timerContainer.add(BorderLayout.SOUTH, timerPickerContainer);
//<editor-fold defaultstate="collapsed" desc="comment">
