...
//                Container parent = firstGroupBy.getParent();
                if (secondGroupByParent != null) {
                    boolean noneSelected = firstGroupBy.getSelectedIndex() == 0;
                    secondGroupByParent.getParent().setHidden(noneSelected); //hide/show container with second field
                    if (!noneSelected) {
                        MyComponentGroup newSecondGroupBy = addSettingEnumAsCompGroup(cont, parseIdMap2, MyPrefs.statisticsSecondGroupBy,
...
//                    if (!noneSelected) {
////                        secondGroupBy.remove();
//                    } else {
//                    parent.replace(secondGroupBy, newSecondGroupBy, null);
//                        secondGroupBy.remove();
//                        int idx = secondGroupByParent.getComponentIndex(firstGroupBy);
//                        secondGroupByParent.addComponent(idx + 1, newSecondGroupBy);
...
//                    secondGroupByParent.revalidate();
                    secondGroupByParent.animateLayout(300);
            ...
            boolean noneSelected = firstGroupBy.getSelectedIndex() == 0;
            secondGroupBy = addSettingEnumAsCompGroup(cont, parseIdMap2, MyPrefs.statisticsSecondGroupBy,
                    ScreenStatistics2.GroupOn2.getSecondLevelGroupingNames(noneSelected ? 0 : firstGroupBy.getSelectedIndex()),
                    ScreenStatistics2.GroupOn2.getSecondLevelGroupingDisplayNames(noneSelected ? 0 : firstGroupBy.getSelectedIndex()), false, true);
