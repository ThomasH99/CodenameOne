...
//        new FieldDef(FIELD_CHALLENGE, "Difficulty", Expr.VALUE_FIELD_TYPE_ENUM),};
    ...
    final static String ITEM = "Task"; //Task";
    ...
    final static String DESCRIPTION_HINT = ITEM; //"Task"; //"New task" DESCRIPTION_HINT"; //Enter New task"; // "Task text"
...
//    final static String COMMENT_HELP = "Use " + COMMENT + " for any additional notes, e.g. indicate progress. You can also automatically add a note when setting a task " + ItemStatus.WAITING + "  depending on the Setting. You can use the DateTimeStamp on the right to easily insert today's date and/or time based on the Setting. Maximum size in free version is XXX**";
    ...
    final static String EFFORT_ESTIMATE_VERY_SHORT = "EST"; //"Estimate" // "Effort estimate";
//    final static String EFFORT_ESTIMATE_HELP = "Estimates let you compare your expected amount of work with your actual amount of work. Is set automatically the first time you enter " + Item.EFFORT_REMAINING + "."; //"Estimate" // "Effort estimate";, "Estimated time"
    ...
    final static String EFFORT_REMAINING_VERY_SHORT = "REM"; //"Remaining effort";
...
//    final static String EFFORT_REMAINING_PROJECT_HELP = EFFORT_REMAINING+" for the project. You can use this to **?? indicate a total for a project before defining its subtasks (or even before realizing that it should be a project)"; //"Effort estimate";"Estimated time (project)"
    ...
    final static String EFFORT_TOTAL_VERY_SHORT = "TOT"; //total effort in Timer (previous actual + timer elapsed time)
    ...
    final static String EARNED_VALUE_PER_HOUR_HELP
            = Format.f("Value divided by time to complete the task ({1 remaining+actual} or {0 estimate})", EFFORT_ESTIMATE, EFFORT_REMAINING + "+" + EFFORT_ACTUAL); //"Value per hour (based on Estimated time)"; //"Value/Effort"; "Value per hour"
...
//    final static String DATE_WHEN_SET_WAITING_HELP = "The time when this task was set "+ItemStatus.WAITING+". Is automatically set. "; // + ItemStatus.WAITING.toString(); //referencing enum String not allowed "Wait until date" "Wait until" "Date when set Waiting"
    ...
    final static String WAITING_ALARM_DATE_HELP = Format.f("Set a special Reminder for a task which is set {0}", ItemStatus.WAITING.getVisibleName()); // 
    final static String SNOOZE_DATE = "Snooze reminder"; // "Waiting alarm"
    final static String WAITING_SNOOZE_DATE = "Snooze waiting"; // "Waiting alarm"
    final static String STARTED_ON_DATE = "Started"; //"Started",  "Date work on tasks started", "Task started on"
    final static String STARTED_ON_DATE_SUBTASKS = "Started (subtasks)"; //"Started",  "Date work on tasks started", "Task started on"
    ...
    final static String FINISH_WORK_TIME_HELP = Format.f("{0} indicates when the task can be finished based on the {1 estimate} and defined {2 work time}. No check mark in the icon means there is not enough {2} to finish it.", FINISH_WORK_TIME, Item.EFFORT_REMAINING, WorkSlot.WORKSLOT);
...
//    final static String URGENCY_HELP = "Indicates the Importance of this task, according to the ** principle. Prioritizing using "+IMPORTANCE+" and "+URGENCY+" can help overcome a tendency to over-prioritize urgent tasks over important tasks.";
    ...
    final static String SHOW_NUMBER_UNDONE_TASKS_TEXT = "Show number of tasks";
    final static String SHOW_NUMBER_UNDONE_TASKS_HELP = "**";
    final static String SHOW_NUMBER_DONE_TASKS_TEXT = "Show number of completed tasks";
    ...
    final static String PARSE_ALARM_SNOOZE_DATE = "alarmSnooze"; //date until which the alarm is snoozed
    final static String PARSE_ALARM_CANCELLED = "alarmCancelled"; //date until which the alarm is snoozed

    final static String PARSE_ALARM2_DATE = "alarm2Date";
    final static String PARSE_ALARM2_SNOOZE_DATE = "alarm2Snooze"; //date until which the alarm is snoozed
    final static String PARSE_ALARM2_CANCELLED = "alarm2Cancelled"; //date until which the alarm is snoozed

    final static String PARSE_WAITING_ALARM_DATE = "waitingAlarmDate";
    final static String PARSE_WAITING_SNOOZE_DATE = "waitingSnooze"; //date until which the alarm is snoozed
    final static String PARSE_WAITING_CANCELLED = "waitingCancelled"; //date until which the alarm is snoozed
...
//    final static String PARSE_PENDING_ALARMS = "pendingAlarms"; //list of alarms (normal, waiting, normal-snoozed, waiting-snoozed) that user have not yet dealt with and that should be shown in AlarmScreen
//    final static String PARSE_WAITING_SNOOZE_DATE = "waitingSnooze"; //date until which the alarm is snoozed
...
//    final static String PARSE_TIMER_STARTED_XXX = "timerStart";
//    final static String PARSE_TIMER_PAUSED_XXX = "timerPaused";
    final static String PARSE_TIMER_TIMESTAMPS = "timerIntervals";
...
//    final static String PARSE_NEXTCOMING_ALARM = "nextAlarm"; //first defined alarm date (may be in the past or future) - the earliest of all defined alarms for the task, used for querying to get tasks with relevant alarms)
//    final static String PARSE_FIRST_ALARM = "firstAlarm"; //last defined alarm date (may be in the past or future). NB. FIRST AND LAST may potentially be the same date
//    final static String PARSE_LAST_ALARM = "lastAlarm"; //first-coming/next-coming alarm (to allow easy search in Parse)
...
//    final static String PARSE_TODAY_VIRT = "today"; //Virtual --> uses FilterSortDef.FILTER_SORT_TODAY_VIEW
...
//</editor-fold>
    ...
    public boolean isDone() {
        //TODO: should check if softDeleted and return true? Hmm, softDeleted should never be included anywhere, but how reliable is the mechanism?!
...
//    }
//</editor-fold>
    public void setOwner(ItemAndListCommonInterface newOwner, boolean updateInheritedValues, boolean removeFromOldOwnerList, boolean addToNewOwner) {
        if (false) {
...
//                        () -> "Setting owner that is not ParseObject or without ObjectId for item=" + this + ", owner=" + owner);
                ASSERT.that(newOwner == null || oldOwner == null || Objects.equals(newOwner, oldOwner) || removeFromOldOwnerList
        ...
        boolean changingOwner = !Objects.equals(newOwner, oldOwner);

//        if (false) { //now done in setOwner()
        if (changingOwner) {
            if (oldOwner != null) {
//                if (changingOwner && updateInheritedValues) {
...
//            if (newOwner != null && changingOwner && updateInheritedValues) { //YES, must update inherited values here (the individual fields will then be updated via the changes stored in opsOnSubtasks
            if (newOwner != null && updateInheritedValues) { //YES, must update inherited values here (the individual fields will then be updated via the changes stored in opsOnSubtasks
                updateValuesInheritedFromOwner(newOwner);
            }
//        }
...
//            if (changingOwner) { //        if (owner instanceof Category) {
            ...
            if (newOwner != null) {
                if (addToNewOwner) {
                    newOwner.addToList(this); //add to default position
                }                //UI: when *changing* owner: already generated repeat instances will remain with old owner, but new ones will be generated in new owner. 
...
//                ownerText = Category.CATEGORY + ": " + ((Category) ownerObj).getText();
    ...
    public String getText() {
        ...
        if (false) {
            String decoded;
            try {
                if (s != null && MyPrefs.encryptTaskTextAndComments.getBoolean()) {
                    decoded = Util.xorDecode(s);
                    if (decoded.startsWith("xx")) {
                        s = decoded;
                    }
                }
            } catch (Exception err) {
...
//            }
            ...
            //WAITING_ALARM
            if ((copyExclusions & COPY_EXCLUDE_WAITING_ALARM_DATE) == 0) {
                destination.setWaitingAlarmDate(getWaitingAlarmDate());
            }
            //EXPIRES ON DATE
...
//        if (defAll || defToRepeatInst || defCopyPaste) {//||defToTempl) { //
        if (false && defAll) {//||defToTempl) { //DONE BELOW
            if (getRemainingForTaskItself() == 0) {
                destination.setRemainingForTaskItself(getRemainingDefaultValue(), false); //getRemainingDefaultValue is only defined if setting already active
            }
        }
...
//            destination.setRemaining(getRemainingForProjectTaskItself(false), false);
            destination.setRemainingForTaskItself(getRemainingForTaskItself(), false);
...
//            }
                if (referenceItem.getHideUntilDateD().getTime() != 0) { //only update if a value was defined for the referenceItem
                    setHideUntilDate(new MyDate(referenceItem.getHideUntilDateD().getTime() + delta));
                }
                ...
                if (referenceItem.getExpiresOnDate().getTime() != 0) { //only update if a value was defined for the referenceItem
                    setExpiresOnDate(referenceItem.getExpiresOnDate().getTime() + delta);
                }
            }
        }
    }
...
//    }
    ...
    public void onDelete(Date deleteDate) {
        ...
        for (Item item : itemsSubtasksOfThisItem) {
            item.onDelete(deleteDate); //let each item delete itself properly, will recurse down the project hierarchy
        }
...
//        }
//</editor-fold>
        listFull.add(index, itemOrListToAdd);
...
//</editor-fold>
        if (false) {
            ((Item) itemOrListToAdd).updateRepeatRule(); //update a RR with pending changes *after* the items is inserted 
        }
        return status;
    }
...
//</editor-fold>
        return addToList((ItemAndListCommonInterface) itemOrListToAdd, MyPrefs.insertNewItemsInStartOfLists.getBoolean());
    }
...
//    }
    @Override
    public boolean addToList(ItemAndListCommonInterface itemOrListToAdd, ItemAndListCommonInterface referenceItemN, boolean addAfterItemOrEndOfList) {
//<editor-fold defaultstate="collapsed" desc="comment">
...
//            return addToList(indexFull + (addAfterItemOrEndOfList ? 1 : 0), newItem);
        return addToList(indexFull, itemOrListToAdd);
    }
...
//    public boolean removeFromList(ItemAndListCommonInterface subItemOrList, boolean removeOwner) {
    public boolean removeFromList(ItemAndListCommonInterface itemOrList, boolean removeOwner) {
        List subtasks = getListFull();
        boolean status = subtasks.remove(itemOrList);
        setList(subtasks);
        assert itemOrList.getOwner() == this : "list not owner of removed subtask, subtask=" + itemOrList + ", owner=" + itemOrList.getOwner() + ", list=" + this;
        if (removeOwner) { // && getOwner() == this)
            itemOrList.setOwner(null);
        }
//        ((Item) itemOrList).removeValuesInheritedFromOwner(this); //now done in setOwnerItem(null)
        return status;
    }
...
//        put(PARSE_TIMER_STARTED_XXX, startTime); //set started time
//        remove(PARSE_TIMER_PAUSED_XXX); //remove any paused time when timer is started
//    }
    public boolean isTimerRunning() {
//        List<Date> timerStartStopsDatesList = getList(PARSE_TIMER_INTERVALS);
        List<Date> timerStartStopsDatesList = getTimerTimeStampListN();
        return timerStartStopsDatesList != null && timerStartStopsDatesList.size() % 2 != 0; //uneven number of startDate, stopDate means timer is running
    }

    /**
     * if timer is currently running for this task, return the timer duration
     * (now - timerStart)
     *
     * @return
     */
    public long getTimerOngoingDurationXXX() {
        List<Date> timerStartStopsDatesList = getList(PARSE_TIMER_TIMESTAMPS);
        if (timerStartStopsDatesList != null && timerStartStopsDatesList.size() % 2 != 0) {
            return new MyDate().getTime() - timerStartStopsDatesList.get(timerStartStopsDatesList.size() - 1).getTime();
        } else {
            return 0;
        }
    }
...
//            addTimerStopTimestamp(startTime);
//        } //else {
//        {
//        if (!isTimerRunning()) {
        List<Date> timerStartStopsDate = getList(PARSE_TIMER_TIMESTAMPS);
        if (timerStartStopsDate == null) {
            timerStartStopsDate = new ArrayList();
        }
...
//            List<Date> timerStartStopsDate = getList(PARSE_TIMER_INTERVALS);
            ...
            timerStartStopsDate.add(stopTime);
//            super.put(PARSE_TIMER_INTERVALS, timerStartStopsDate);
            setTimerTimeStampListN(timerStartStopsDate);
...
//        List<Date> timerStartStopsDate = getList(PARSE_TIMER_INTERVALS);
        ...
        long duration = 0;
        if (timerStartStopsDate != null) {
            for (int i = 0, size = timerStartStopsDate.size(); i < size; i += 2) {
                if (i <= size - 2) {
                    ASSERT.that(timerStartStopsDate.get(i + 1).getTime() - timerStartStopsDate.get(i).getTime() >= 0);
                    duration += timerStartStopsDate.get(i + 1).getTime() - timerStartStopsDate.get(i).getTime();
                } else if (i < size && includeRunningTimerUpToNow) { //if an orphan startTime is in list
                    duration += timerStartStopsDate.get(i).getTime() - timerStartStopsDate.get(i).getTime();
                    break;
                }
            }
        }
...
//    public void timerPauseXXX(Date pauseTime) {
//        Date timerStarted = getDate(PARSE_TIMER_STARTED);
//        if (timerStarted != null) {
//            throw new RuntimeException("Paused when no timer was running");
//        }
//        put(PARSE_TIMER_PAUSED_XXX, pauseTime); //set timer to now
//    }
//
//    public boolean timerIsPausedXXX() {
//        return get(PARSE_TIMER_PAUSED_XXX) != null; //set timer to now
//    }
//
//    public long timerRunningTimeXXX() {
//        Date timerStarted = getDate(PARSE_TIMER_STARTED);
//        Date timePaused = getDate(PARSE_TIMER_PAUSED_XXX);
...
//        long timeSpent = timerRunningTimeXXX();
//        remove(PARSE_TIMER_STARTED);
//        remove(PARSE_TIMER_PAUSED_XXX);
//        setActualForTaskItself(getActualForTaskItself()+ timeSpent,true); //store new actual effort
//    }
...
//        return isInherited(isStarred(), potentiallyInheritedValue, MyPrefs.itemInheritOwnerStarredProperties.getBoolean());
        if (getOwnerItem() != null) {
//            return isInherited(getOwnerItem().isStarred(), potentiallyInheritedValue, MyPrefs.itemInheritOwnerProjectStarred.getBoolean());
            return isInherited(getOwnerItem().isStarred(), potentiallyInheritedValue, isStarredInheritanceOn());
        } else {
            return false;
        }
    }
...
//            return isInherited(getOwnerItem().getPriority(), potentiallyInheritedValue, MyPrefs.itemInheritOwnerProjectPriority.getBoolean());
            return isInherited(getOwnerItem().getPriority(), potentiallyInheritedValue, isPriorityInheritanceOn());
        } else {
            return false;
        }
...
//            return isInherited(getOwnerItem().getChallengeN(), potentiallyInheritedValue, MyPrefs.itemInheritOwnerProjectChallenge.getBoolean());
            return isInherited(getOwnerItem().getChallengeN(), potentiallyInheritedValue, isChallengeInheritanceOn());
        } else {
            return false;
        }
...
//<editor-fold defaultstate="collapsed" desc="comment">
//        if (false && MyUtil.neql(challenge, oldVal)) {
//            updateSubtasksWithInheritedValues((subtask) -> {
//                if (MyUtil.eql(subtask.getChallengeN(), oldVal) && subtask.updateInheritedValuesFor(PARSE_CHALLENGE)) { //if old project value equals current subtask value, then update subtasks value to project's new value
//                    subtask.setChallenge(challenge);
//                    return true;
//                }
...
//            return isInherited(getOwnerItem().getDreadFunValueN(), potentiallyInheritedValue, MyPrefs.itemInheritOwnerProjectDreadFun.getBoolean());
            return isInherited(getOwnerItem().getDreadFunValueN(), potentiallyInheritedValue, isDreadFunInheritanceOn());
        } else {
            return false;
        }
    }
...
//    private List<UpdateItem> opsToUpdateSubtasks = new ArrayList(); //list of operations to execute 
    private HashMap<String, UpdateItem> opsUpdateInheritedValues = new HashMap<String, UpdateItem>(); //list of operations to execute 
    private List<Runnable> opsUpdateRepeatRule = new ArrayList(); //operations to run once all changes to Item's fields have been made, e.g. repeatRules
    private boolean updateOwnerItem = false;
//    private List<UpdateItem> opsUpdateRepeatRule = new ArrayList(); //operations to run once all changes to Item's fields have been made, e.g. repeatRules
...
//            return isInherited(getOwnerItem().getImportanceN(), potentiallyInheritedValue, MyPrefs.itemInheritOwnerProjectImportance.getBoolean());
            return isInherited(getOwnerItem().getImportanceN(), potentiallyInheritedValue, isImportanceInheritanceOn());
        } else {
            return false;
        }
    }
...
//            return isInherited(getOwnerItem().getUrgencyN(), potentiallyInheritedValue, MyPrefs.itemInheritOwnerProjectUrgency.getBoolean());
            return isInherited(getOwnerItem().getUrgencyN(), potentiallyInheritedValue, isUrgencyInheritanceOn());
        } else {
            return false;
        }
    }
...
//  
    ...
    public static void sortAlarmRecords(List<AlarmRecord> alarmRecords) {
        if (alarmRecords.size() > 1) {
            Collections.sort(alarmRecords, (object1, object2) -> {
                return FilterSortDef.compareDate(object2.alarmTime, object1.alarmTime); //show most recent alarm on top of list
            });
        }
    }

    /**
     * returns all alarms, after or equal afterDate, sorted by date if sorted is
     * true. Returns empty list if no alarms defined after onOrAfterDate.
     * Returns dates whether Item is Done or not.
     *
     * @return
     */
//<editor-fold defaultstate="collapsed" desc="comment">
//    public List<AlarmRecord> getAllAlarmRecordsOLD(Date onOrAfterDate, boolean sorted) {
...
//            list.add(new AlarmRecord(date, notification,this));
//        }
////        if ((date = getWaitingAlarmDateD()) != null && (onOrAfterDate == null || date.getTime() >= onOrAfterDate.getTime())) {
//        if ((date = getWaitingAlarmDate()).getTime() != 0 && (onOrAfterDate == null || date.getTime() >= onOrAfterDate.getTime())) {
//            list.add(new AlarmRecord(date, waiting,this));
//        }
...
//</editor-fold>
    public List<AlarmRecord> getAllAlarmRecords(Date onOrAfterDate, boolean sorted) {
        ...
        //get normal alarms - if snoozed only that, if not snoozed, only normal alarm time if not cancelled
        if ((date = getSnoozeAlarm()).getTime() != 0 && (date.getTime() >= onOrAfterDate.getTime())) {
            list.add(new AlarmRecord(date, AlarmType.snoozedNotif, this));
        } else if (!isAlarmCancelledOrSnoozed() && (date = getAlarmDate()).getTime() != 0 && (date.getTime() >= onOrAfterDate.getTime())) {
            list.add(new AlarmRecord(date, AlarmType.notification, this));
        }

        //get waiting alarms - if snoozed only that, if not snoozed, only normal waiting alarm time if not cancelled
        if ((date = getSnoozeWaitingAlarm()).getTime() != 0 && date.getTime() >= onOrAfterDate.getTime()) {
            list.add(new AlarmRecord(date, AlarmType.snoozedWaiting, this));
        } else if (!isWaitingAlarmCancelledOrSnoozed() && (date = getWaitingAlarmDate()).getTime() != 0 && date.getTime() >= onOrAfterDate.getTime()) {
            list.add(new AlarmRecord(date, AlarmType.waiting, this));
        }
...
//</editor-fold>
        if (sorted) {
            sortAlarmRecords(list);
        }
        return list;
    }

    public List<AlarmRecord> getAllFutureAlarmRecordsSorted() {
        return getAllFutureAlarmRecordsSorted(false);
    }

    public List<AlarmRecord> getAllFutureAlarmRecordsSorted(boolean evenIfDone) {
        if (!evenIfDone && isDone()) {
            return new ArrayList();
        }
        return getAllAlarmRecords(new MyDate(), true);
    }

    public List<AlarmRecord> getAllFutureAlarmRecordsUnsorted(Date afterDate) {
        if (isDone()) {
            return new ArrayList();
        }
        return getAllAlarmRecords(afterDate == null ? new MyDate() : afterDate, false);
    }

    public List<AlarmRecord> getAllFutureAlarmRecordsUnsorted() {
        return getAllFutureAlarmRecordsUnsorted(new MyDate());
    }

    /**
     * return alarm records for expired and yet unprocessed alarms, ie either a
     * snoozed alarm in the past (which needs to be deleted when cancelled), or
     * if no snooze, an alarm in the past which is not yet cancelled
     *
     * @param firstDate only return alarms after this date (ignore very old
     * alarms)
     * @param beforeDate only return alarms before this date (typically 'now')
     * @param sorted
     * @return
     */
    public List<AlarmRecord> getUnprocessedAlarmRecordsXXX(Date firstDate, Date beforeDate, boolean sorted) {
        //TODO //optimization: keep track of whether alarms have been changed to avoid recalculating this at every  call of getNextcomingAlarm() //NO, probably very little to be gained
        List<AlarmRecord> list = new ArrayList();
        Date date;

        //if snoozed, return snooze if expired, if no snooze return normal alarms if expired AND cancelled
        //NB. If snooze is in the future, must not return the cancelled alarm date
        if ((date = getSnoozeAlarm()).getTime() != 0) {
            if (date.getTime() <= beforeDate.getTime() && date.getTime() >= firstDate.getTime()) {
                list.add(new AlarmRecord(date, AlarmType.snoozedNotif, this));
            }
        } else if (!isAlarmCancelledOrSnoozed() && (date = getAlarmDate()).getTime() != 0
                && date.getTime() <= beforeDate.getTime() && date.getTime() >= firstDate.getTime()) {
            list.add(new AlarmRecord(date, AlarmType.notification, this));
        }

        //if snoozed, return snooze if expired, if no snooze return normal waiting alarm if expired AND cancelled
        //NB. If snooze is in the future, must not return the cancelled alarm date
        if (((date = getSnoozeWaitingAlarm()).getTime()) != 0) {
            if (date.getTime() <= beforeDate.getTime() && date.getTime() >= firstDate.getTime()) {
                list.add(new AlarmRecord(date, AlarmType.snoozedWaiting, this));
            }
        } else if (!isWaitingAlarmCancelledOrSnoozed() && (date = getWaitingAlarmDate()).getTime() != 0
                && date.getTime() <= beforeDate.getTime() && date.getTime() >= firstDate.getTime()) {
            list.add(new AlarmRecord(date, AlarmType.waiting, this));
        }
    ...
    /**
     * get the list of unprocessed alarms for this task (expired and not (yet)
     * cancelled or snoozed). Returns empty list for Completed/Cancelled tasks.
     *
     * @return
     */
//<editor-fold defaultstate="collapsed" desc="comment">
//    public List<AlarmRecord> getUnprocessedAlarms() {
//        List<AlarmRecord> unprocessedAlarms = new ArrayList();
//        String flags = getString(PARSE_PENDING_ALARMS);
//        if (flags != null && flags.length() > 0) {
//            if (flags.indexOf(ALARM_NOTIF_UNPROCESSED) != -1) {
//                if (getSnoozeAlarm().getTime() != 0) {
//                    unprocessedAlarms.add(new AlarmRecord(getSnoozeAlarm(), notification));
//                } else if (getAlarmDate().getTime() != 0) {
//                    unprocessedAlarms.add(new AlarmRecord(getAlarmDate(), notification));
//                }
//            }
//            if (flags.indexOf(ALARM_WAITING_UNPROCESSED) != -1) {
//                if (getSnoozeWaitingAlarm().getTime() != 0) {
//                    unprocessedAlarms.add(new AlarmRecord(getSnoozeWaitingAlarm(), notification));
//                } else if (getWaitingAlarmDate().getTime() != 0) {
//                    unprocessedAlarms.add(new AlarmRecord(getWaitingAlarmDate(), notification));
//                }
//            }
//        }
...
//        return getUnprocessedAlarms(new MyDate(), false);
//        Date now = new MyDate();
        Date last = DAO.getLastUnprocessedAlarmDate(now);
        return getUnprocessedAlarms(DAO.getFirstUnprocessedAlarmDate(last), last, false);
    }

    public List<AlarmRecord> getUnprocessedAlarms(Date firstDate, Date lastDate, boolean sorted) {
        List<AlarmRecord> unprocessedAlarms = new ArrayList();
        if (isDone()) {
            return unprocessedAlarms;
        }
//        Date oldestDateToFetch = new MyDate(lastDate.getTime() - MyPrefs.alarmPastDaysToFetchUncancelledAlarms.getInt() * MyDate.DAY_IN_MILLISECONDS);
//        Date firstDate = firstDate;
        {
            Date date;
            if (MyDate.isAfter(date = getSnoozeAlarm(), firstDate) && MyDate.isBefore(date, lastDate)) {// &&!isAlarmCancelled()) { //NO don't check for Cancelled since Alarm is cancelled first time alarm is snoozed and Snnoze is deleted when cancelled
                unprocessedAlarms.add(new AlarmRecord(date, AlarmType.snoozedNotif, this)); //snooze alarm has priority over alarm
            } else if (!isAlarmCancelledOrSnoozed() && MyDate.isAfter(date = getAlarmDate(), firstDate) && MyDate.isBefore(date, lastDate)) {
                unprocessedAlarms.add(new AlarmRecord(date, AlarmType.notification, this));
            }
        }
        {
            Date date;
            if (MyDate.isAfter(date = getSnoozeWaitingAlarm(), firstDate) && MyDate.isBefore(date, lastDate)) {
                unprocessedAlarms.add(new AlarmRecord(date, AlarmType.snoozedWaiting, this));
            } else if (!isWaitingAlarmCancelledOrSnoozed() && MyDate.isAfter(date = getWaitingAlarmDate(), firstDate) && MyDate.isBefore(date, lastDate)) {
                unprocessedAlarms.add(new AlarmRecord(date, AlarmType.waiting, this));
            }
        }
//SORT
        if (sorted) {
            sortAlarmRecords(unprocessedAlarms);
        }

        return unprocessedAlarms;
    }

//    public List<AlarmRecord> getUnprocessedAlarmRecordsUnosrted() {
//        Date now = new MyDate();
//        Date first = new MyDate(now.getTime() - MyPrefs.)
//        return getUnprocessedAlarmRecords(firstDate,, false)
//    }
//
...
//    }
     ...
     * @return
     */
    public Date getNextFutureAlarmN() {
...
//    }
//</editor-fold>
//    public void updateNextcomingAlarmXXX() {
//        Date date = getNextcomingAlarmN(); //List<AlarmRecord> list = getAllFutureAlarmRecordsSorted();
//        setNextcomingAlarmDate(date);
////        Date oldVal = getDate(PARSE_NEXTCOMING_ALARM);
////        if (date != null && date.getTime() != 0) { //list.isEmpty()) {
...
//    }
    /**
     * call on every change to any alarms. NB. No need to have getters for
     * First/Last since we'll always access the alarms via
     * getAllFutureAlarmRecordsSorted(). And First/Last are only used to query
     * Parse for items which may have relevant alarms
     *
     * @param firstAlarmDate
     */
//    private void updateFirstAndLastAlarm() {
//        List<AlarmRecord> allAlarms = getAllFutureAlarmRecordsSorted();
//        if (allAlarms.size() > 0) {
//            put(PARSE_FIRST_ALARM, allAlarms.get(0).alarmTime);
//            put(PARSE_LAST_ALARM, allAlarms.get(allAlarms.size() - 1).alarmTime);
//        }
//    }
...
//</editor-fold>
        ...
        if (alarmDate != null && alarmDate.getTime() != 0) {
            if (!Objects.equals(oldAlarmDate, alarmDate)) {
                put(PARSE_ALARM_DATE, alarmDate);
                //if new alarmDate is in the past, then automatically Cancel the alarm and cancel any pending Snooze
                if (alarmDate.getTime() <= System.currentTimeMillis()) {
                    setSnoozeAlarm(null);
                    cancelOrSnoozeAlarm(true);
                } else { //if new date is in the future, then remove any Cancel status as well as any pending Snooze
                    setSnoozeAlarm(null);
                    cancelOrSnoozeAlarm(false);
                }
            }
        } else {
            remove(PARSE_ALARM_DATE);
            setSnoozeAlarm(null);
            cancelOrSnoozeAlarm(false); //remove any cancel flag if alarm is removed (shouldn't be necessary, but better to clean up)
        }
    }
...
//    }
//    public void setSnoozeAlarmRecordXXX(AlarmRecord snooze) {
...
////        }
//        if (snooze.alarmTime != null && snooze.alarmTime.getTime() != 0) {
//            setSnoozeAlarm(snooze.alarmTime);
//        } else {
//            setSnoozeAlarm(null);
//        }
//    }
//</editor-fold>
    public void setSnoozeAlarm(Date snoozeTillDate) {
        Date oldSnoozeDate = getSnoozeAlarm();
        if (snoozeTillDate != null && snoozeTillDate.getTime() != 0) {
            if (!Objects.equals(oldSnoozeDate, snoozeTillDate)) {
                put(PARSE_ALARM_SNOOZE_DATE, snoozeTillDate);
                //if new alarmDate is in the past, then automatically Cancel the alarm and cancel any pending Snooze
                ASSERT.that(snoozeTillDate.getTime() > System.currentTimeMillis(), "snooze date should always be in the future");
            }
            cancelOrSnoozeAlarm(true); //whenever an alarm is snoozed (or snooze is deleted), this overrides and cancels the normal alarm (which will only become active again if updated to a future date)
        } else {
            remove(PARSE_ALARM_SNOOZE_DATE);
        }
    }

    public Date getSnoozeAlarm() {
        Date date = getDate(PARSE_ALARM_SNOOZE_DATE);
        return (date != null) ? date : new MyDate(0);
    }

    public void cancelOrSnoozeAlarm(boolean cancel) {
        if (cancel) {
            put(PARSE_ALARM_CANCELLED, cancel);
//            remove(PARSE_ALARM_SNOOZE_DATE); //remove snooze date when alarm is cancelled //NO, don't remove snooze, Alarm will be cancelled when setting a Snooze
        } else {
            remove(PARSE_ALARM_CANCELLED);
        }
    }

    public boolean isAlarmCancelledOrSnoozed() {
        Boolean cancelled = getBoolean(PARSE_ALARM_CANCELLED);
        return cancelled != null; //by default (empty value), means NOT cancelled, so empty or TRUE mean cancelled
    }
...
//    }
//</editor-fold>
    public Date getWaitingAlarmDate() {
    ...
    public void setWaitingAlarmDate(Date waitingAlarmDate) {
        ...
        if (waitingAlarmDate != null && waitingAlarmDate.getTime() != 0) {
            if (!Objects.equals(oldAlarmDate, waitingAlarmDate)) {
                ...
                //if new alarmDate is in the past, then automatically Cancel the alarm and cancel any pending Snooze
                if (waitingAlarmDate.getTime() <= System.currentTimeMillis()) {
                    setSnoozeWaitingAlarm(null);
                    cancelOrSnoozeWaitingAlarm(true);
                } else { //if new date is in the future, then remove any Cancel status as well as any pending Snooze
                    setSnoozeWaitingAlarm(null);
                    cancelOrSnoozeWaitingAlarm(false);
                }
            }
        } else if (oldAlarmDate.getTime() != 0) {
            remove(PARSE_WAITING_ALARM_DATE);
            setSnoozeWaitingAlarm(null); //renove any pending snooze
            cancelOrSnoozeWaitingAlarm(false); //renove any cancel state (clean up, not fonctionally needed)
        }
    }

    public void setSnoozeWaitingAlarm(Date snoozeWaitingDate) {
        Date oldSnoozeDate = getSnoozeWaitingAlarm();
        if (snoozeWaitingDate != null && snoozeWaitingDate.getTime() != 0) {
            if (!Objects.equals(oldSnoozeDate, snoozeWaitingDate)) {
                put(PARSE_WAITING_SNOOZE_DATE, snoozeWaitingDate);
                //if new alarmDate is in the past, then automatically Cancel the alarm and cancel any pending Snooze
                ASSERT.that(snoozeWaitingDate.getTime() > System.currentTimeMillis(), "snooze date should always be in the future");
                cancelOrSnoozeWaitingAlarm(true); //whenever an alarm is snoozed (or snooze is deleted), this overrides and cancels the normal alarm (which will only become active again if updated to a future date)
            }
        } else {
            remove(PARSE_WAITING_SNOOZE_DATE);
            //don't cancelWaitingAlarm(true) here since setSnooze might be called without having set a snooze
        }
    }

    public Date getSnoozeWaitingAlarm() {
        Date date = getDate(PARSE_WAITING_SNOOZE_DATE);
        return (date != null) ? date : new MyDate(0);
    }

    public void cancelOrSnoozeWaitingAlarm(boolean cancel) {
        if (cancel) {
            put(PARSE_WAITING_CANCELLED, cancel);
//            remove(PARSE_WAITING_SNOOZE_DATE); //remove snooze date when alarm is cancelled //NO, don't remove snooze, Alarm will be cancelled when setting a Snooze
        } else {
            remove(PARSE_WAITING_CANCELLED);
        }
    }

    public boolean isWaitingAlarmCancelledOrSnoozed() {
        Boolean cancelled = getBoolean(PARSE_WAITING_CANCELLED);
        return cancelled != null; //by default (empty value), means NOT cancelled, so empty or TRUE mean cancelled
    }
     ...
     * @return
     */
    public RepeatRuleParseObject getRepeatRuleN() {
//        return repeatRule;
...
//                if (!oldRepeatRule.equals(newRepeatRuleN)) { //do nothing if rule is not edited!!
            if (newRepeatRuleN == null) { //when will this happen?! What about deleting the (possibly) now unreferenced RR?
                setRepeatRuleInParse(null);
            } else {
                if (newRepeatRuleN.isDirty()) { //do nothing if rule is not edited!!
                    newRepeatRuleN.setUpdatePending(!isTemplate());
                    setRepeatRuleInParse(newRepeatRuleN); //set to ensure it is saved
                }
            }
        }
    }
...
//    List<ItemAndListCommonInterface> getLeafTasksAsList(Condition condition) {
    @Override
    public List<Item> getLeafTasksAsListN(Condition condition) {
//        assert previousItem != null || previousItemAlreadyFound[0] : "getNextLeafItemMeetingConditionImpl called with previousItem==null and previousItemAlreadyFound not set true";
        if (!isProject()) { //LEAF
            ...
            if (size > 0) {
                ...
                for (int i = 0; i < size; i++) {
                    leaftasks = subtasks.get(i).getLeafTasksAsListN(condition);
...
//</editor-fold>
        if (isProject()) {
            if (countLeafTasks) {
                ...
                return isDone() && onlyUndone ? 0 : 1; //this is a project, but since we don't count leafTask, we only count it if undone
            }
        } else {
            return isDone() && onlyUndone ? 0 : 1; //this is a leafTask, so always return 1 if undone
        }
        //!(onlyUndone && isDone()) <=> !onlyUndone || !isDone()
    }
...
//            }
//</editor-fold>
        if (!item.isProject()) { //don't set completed/started for a project since the values comes from the subtasks
            //StartedOnDate SET:
...
//            }
        ...
        if (previousStatus != ItemStatus.WAITING && newStatus == ItemStatus.WAITING) { //UI: always only save the last time the task was set Waiting //-only save the first setWaitingDate (TODO!!!: or is it more intuitive that it's the last, eg it set waiting by mistake?)
            item.setDateWhenSetWaiting(now, false); //always save. false: don't auto-update add'l dependent fields
        }
        ...
        if (previousStatus == ItemStatus.WAITING && newStatus != ItemStatus.WAITING) { //UI: always only save the last time the task was set Waiting //-only save the first setWaitingDate (TODO!!!: or is it more intuitive that it's the last, eg it set waiting by mistake?)
            ...
            if (newStatus == ItemStatus.CREATED || newStatus != ItemStatus.ONGOING) {
                item.setDateWhenSetWaiting(null, false); //reset the date if task is set back to Created/Ongoing, but NOT if set directly to Completed
            }
        }
//<editor-fold defaultstate="collapsed" desc="deactivated updates">
        if (false && (previousStatus == ItemStatus.WAITING && newStatus != ItemStatus.WAITING && item.getWaitUntilDate().getTime() != 0L)) { //reset WaitingTillDate
            ...
            item.setWaitUntilDate(null); //reset waitingTill date
...
////</editor-fold>
    ...
    public boolean confirmUpdateOfSubtasks(ItemStatus oldStatus, ItemStatus newStatus) {
        ...
        return (nbChgStatus <= MyPrefs.itemMaxNbSubTasksToChangeStatusForWithoutConfirmation.getInt()
                ...
                || Dialog.show("INFO",
                        Format.f("This will change status to {0 newstatus} for {1 nbTasksChanged} subtasks in project \"{2 prjName}\"", newStatus.getVisibleName(), "" + nbChgStatus, getText()),
                        "OK", "Cancel"));
    }
...
//        else return getStatus();
     ...
     *
     * @param status
     * @param updateSubtasks
     * @param updateSupertasks
     * @param updateDependentFields
     * @param now
     * @param executeRepeat
     * @param askToUpdateWaitingDateAndAlarm
     * @param forceUpdateOfSubtasksZZZ no reason to use this, subtasks should
     * always be updated when updateSubtasks is true. Old use: override the
     * confirmation dialog when changing many subtasks in one go, currently if
     * marking an entire list of tasks completed.
     * @return
     */
    public boolean setStatus(final ItemStatus status, boolean updateSubtasks, boolean updateSupertasks, boolean updateDependentFields, Date now,
            boolean executeRepeat, boolean askToUpdateWaitingDateAndAlarm, boolean forceUpdateOfSubtasksZZZ) {
        ItemStatus oldStatus = getStatusFromParse();
...
//        } else {
        if (isProject() && updateSubtasks) {
            if (true || forceUpdateOfSubtasksZZZ || confirmUpdateOfSubtasks(oldStatus, newStatus)) { //true: de-activated the confirmation call (and then forceUpdateOfSubtasks also!)
                //when changing the status of a project, only the status of the subtasks are changed(??)
//            boolean doneProject = (oldStatus == ItemStatus.DONE);
//            int nbChgStatus = getNumberOfItemsThatWillChangeStatus(true, newStatus, doneProject);
//            if (nbChgStatus <= MyPrefs.itemMaxNbSubTasksToChangeStatusForWithoutConfirmation.getInt()
...
//                    subtask.setStatus(newStatus, true, false, true, now); //always update dependent fields for subtasks
                        //askToUpdateWaitingDateAndAlarm==false here to avoid asking to set waiting alarms etc for each subtask!!
//                        subtask.setStatus(newStatus, true, true, true, now, true, false, false); //always update dependent fields for subtasks. MUST also update supertask so that e.g. the completedDate of project is updated with latest subtask date, must update subtask repeatRules!
...
MUST also update supertask so that e.g. the completedDate of project is updated with latest subtask date*/
                        subtask.setStatus(newStatus, true, false, true, now, true, false, false);
                        return true;
                    } else {
        ...
        if (askToUpdateWaitingDateAndAlarm && newStatus == ItemStatus.WAITING && MyPrefs.waitingAskToSetWaitingDateWhenMarkingTaskWaiting.getBoolean()) {
            MyForm.showDialogSetWaitingDateAndAlarmIfAppropriate(this); //only call if we're changing TO Waiting status
        }
...
//<editor-fold defaultstate="collapsed" desc="comment">
//if (false && updateSupertasks) { //must call *after* creating repeat instances to update project correctly based on new subtasks
//    updateOwnerItem = true;
//    Item owner = getOwnerItem();
//    if (owner != null) {// && owner.getStatus() != newStatus) {
////                owner.updateStatusOnSubtaskStatusChange(this, oldStatus, newStatus, now);
...
//        }
//</editor-fold>
        ItemStatus itemStatus = getStatusFromParse();
//        long actualTotalEffortForProject = getActualTotal();
//        if (actualTotalEffortForProject > 0 &&itemStatus==ItemStatus.CREATED) {
//            setSt
//            return getStatusFromParse();
//        }
        return itemStatus;
...
//                if (MyPrefs.itemInheritOwnerProjectChallenge.getBoolean() && getChallengeN() == null) { //false=> challenge is not making a lot of sense to inherit for each subtask (a challenging project may have simply subtasks)
                if (isChallengeInheritanceOn() && getChallengeN() == null) { //false=> challenge is not making a lot of sense to inherit for each subtask (a challenging project may have simply subtasks)
...
//                if (MyPrefs.itemInheritOwnerProjectDreadFun.getBoolean() && getDreadFunValueN() == null) { //false=> challenge is not making a lot of sense to inherit for each subtask (a challenging project may have simply subtasks)
                if (isDreadFunInheritanceOn() && getDreadFunValueN() == null) { //false=> challenge is not making a lot of sense to inherit for each subtask (a challenging project may have simply subtasks)
...
//            if (MyPrefs.itemInheritOwnerProjectPriority.getBoolean() && getPriority() == previousOwnerItem.getPriority()) {
            if (isPriorityInheritanceOn() && getPriority() == previousOwnerItem.getPriority()) {
                setPriority(0);
            }

//            if (MyPrefs.itemInheritOwnerProjectUrgency.getBoolean() && MyUtil.eql(getUrgencyN(), previousOwnerItem.getUrgencyN())) {
            if (isUrgencyInheritanceOn() && MyUtil.eql(getUrgencyN(), previousOwnerItem.getUrgencyN())) {
                setUrgency(null);
            }

//            if (MyPrefs.itemInheritOwnerProjectImportance.getBoolean() && MyUtil.eql(getImportanceN(), previousOwnerItem.getImportanceN())) {
            if (isImportanceInheritanceOn() && MyUtil.eql(getImportanceN(), previousOwnerItem.getImportanceN())) {
                setImportance(null);
            }
...
//            if (MyPrefs.itemInheritOwnerProjectChallenge.getBoolean() && MyUtil.eql(getChallengeN(), previousOwnerItem.getChallengeN())) { //false=> challenge is not making a lot of sense to inherit for each subtask (a challenging project may have simply subtasks)
            if (isChallengeInheritanceOn() && MyUtil.eql(getChallengeN(), previousOwnerItem.getChallengeN())) { //false=> challenge is not making a lot of sense to inherit for each subtask (a challenging project may have simply subtasks)
                setChallenge(null);
            }
...
//            if (MyPrefs.itemInheritOwnerProjectDreadFun.getBoolean() && MyUtil.eql(getDreadFunValueN(), previousOwnerItem.getDreadFunValueN())) { //false=> challenge is not making a lot of sense to inherit for each subtask (a challenging project may have simply subtasks)
            if (isDreadFunInheritanceOn() && MyUtil.eql(getDreadFunValueN(), previousOwnerItem.getDreadFunValueN())) { //false=> challenge is not making a lot of sense to inherit for each subtask (a challenging project may have simply subtasks)
...
//            return isInherited(getOwnerItem().getHideUntilDateD(), potentiallyInheritedValue, MyPrefs.itemInheritOwnerProjectHideUntilDate.getBoolean());
            return isInherited(getOwnerItem().getHideUntilDateD(), potentiallyInheritedValue, isHideUntilDateInheritanceOn());
        } else {
            return false;
        }
    }
...
//            return isInherited(getOwnerItem().getStartByDateD(), potentiallyInheritedValue, MyPrefs.itemInheritOwnerProjectStartByDate.getBoolean());
            return isInherited(getOwnerItem().getStartByDateD(), potentiallyInheritedValue, isStartByInheritanceOn());
        } else {
            return false;
        }
    }
...
//    final static char ALARM_NOTIF_UNPROCESSED = 'A'; //flag 
//    final static int ALARM_WAITING_UNPROCESSED = 'W';
//list of alarms (normal, waiting, normal-snoozed, waiting-snoozed) that user have not yet dealt with and that should be shown in AlarmScreen
    /**
     * store this alarm has expired on a device and should be shown in
     * AlarmScreen until the user has either Cancelled it, or Snoozed it.
     *
     * @param alarmType
     */
//    public void setUnprocessedAlarm(AlarmType alarmType, boolean set) {
//        String unprocessedFlags = getString(PARSE_PENDING_ALARMS);
//        if (unprocessedFlags == null) {
//            unprocessedFlags = "";
//        }
//
//        char flag = alarmType == notification ? ALARM_NOTIF_UNPROCESSED : ALARM_WAITING_UNPROCESSED;
//
//        if (set) {
//            int idx = unprocessedFlags.indexOf(flag);
//            ASSERT.that(idx == -1, "alarmFlag shouldn't already be set, flags=" + unprocessedFlags + "; new flag=" + flag);
//            if (idx != -1) {
//                unprocessedFlags += alarmType == notification ? ALARM_NOTIF_UNPROCESSED : ALARM_WAITING_UNPROCESSED;
//            }
//        } else {
//            unprocessedFlags = MyUtil.replaceSubstring(unprocessedFlags, "" + flag, "");
//        }
//        if (unprocessedFlags.length() > 0) {
//            put(PARSE_PENDING_ALARMS, unprocessedFlags);
//        } else {
//            remove(PARSE_PENDING_ALARMS);
//        }
//    }
//
//    public String getUnprocessedAlarmFlags() {
//        String unprocessedFlags = getString(PARSE_PENDING_ALARMS);
//        return unprocessedFlags == null ? "" : unprocessedFlags;
//    }
...
//    }
    public void setWaitUntilDateInParse(Date waitUntilDate) {
        setWaitUntilDateInParse(waitUntilDate, false, true, true);
    }

    public void setWaitUntilDateInParse(Date waitUntilDate, boolean updateWaitingAlarm, boolean updateItemStatus, boolean updateDateWhenSetWaiting) {
//<editor-fold defaultstate="collapsed" desc="comment">
//        if (isProject()) {
...
//</editor-fold>
        ...
        if (waitUntilDate != null && waitUntilDate.getTime() != 0) {
            if (!Objects.equals(oldVal, waitUntilDate)) {
                ...
                if (updateItemStatus && MyPrefs.waitingSetStatusEtcWhenSettingWaitingUntilDate.getBoolean()
                        && (oldStatus == ItemStatus.CREATED || oldStatus == ItemStatus.ONGOING)) { //only update if created or ongoing (not cancelled, Done)
//                    setStatus(ItemStatus.WAITING);
                    setStatus(ItemStatus.WAITING, true, true, true, new MyDate(), true, false, false); //only update status and other dependent field, but don't show dialog to update waiting
                }

//                    if (getWaitingAlarmDate().getTime() == 0 && MyPrefs.waitingSetStatusEtcWhenSettingWaitingUntilDate.getBoolean()) {
                if (updateWaitingAlarm && getWaitingAlarmDate().getTime() == 0
                ...
                if (updateDateWhenSetWaiting) {
                    setDateWhenSetWaiting(new MyDate(), false); //set waiting now, false=>
                }
            }
        } else if (oldVal != null) {
            remove(PARSE_WAIT_UNTIL_DATE);
            if (updateItemStatus && MyPrefs.waitingSetStatusEtcWhenDeletingWaitingUntilDate.getBoolean() && oldStatus == ItemStatus.WAITING) { //only update if created or ongoing (not cancelled, Done)
                ...
                if (false && updateDateWhenSetWaiting) { //UI: leave first setWaiting date for history
                    setDateWhenSetWaiting(null);
                }
            }
        }
//        update();
...
//TODO!!! check that date is set when status is set Waiting
        ...
        if (MyPrefs.itemInheritOwnerProjectDateWhenSetWaitingZZZ.getBoolean() && !Objects.equals(waitingLastActivatedDate, oldVal)) {
...
//                if (eql(getImportanceN(), subtask.getImportanceN())) { //if old project value equals current subtask value, then update subtasks value to project's new value
        ...
        if (waitingLastActivatedDate != null && waitingLastActivatedDate.getTime() != 0) {
            if (!Objects.equals(oldVal, waitingLastActivatedDate)) {
                ...
                if (autoUpdateDependentFields && MyPrefs.waitingSetStatusWaitingWhenSettingDateWhenWaiting.getBoolean() && (oldStatus == ItemStatus.CREATED || oldStatus == ItemStatus.ONGOING)) { //only update if created or ongoing (not cancelled, Done)
                    setStatus(ItemStatus.WAITING, autoUpdateDependentFields);
                }
            }
        } else {
            remove(PARSE_DATE_WHEN_SET_WAITING);
            if (autoUpdateDependentFields && MyPrefs.waitingSetStatusAwayFromWaitingWhenRemovingSettingDateWhen.getBoolean() && oldStatus == ItemStatus.WAITING) { //only update if created or ongoing (not cancelled, Done)
                setStatus(getActualTotal() != 0 ? ItemStatus.ONGOING : ItemStatus.CREATED, autoUpdateDependentFields); //UI: if deleting setWaitingDate, then reset status to whatever it was before
            }
        }
    }
...
//        long oldVal = getLong(PARSE_EFFORT_ESTIMATE);
        ...
        if (effortEstimateMillis != oldVal) {
...
//        setRemainingForProjectTaskItselfInParse(getRemainingDefaultValue());
        if (getRemainingTotal() == 0) {
            setRemainingForTaskItself(getRemainingDefaultValue());
        }
    }
...
//    }
//</editor-fold>
    public void setRemainingForTaskItself(long remainingEffortForTaskItselfMillis, boolean autoUpdateEffortEstimate) {
        setRemainingForTaskItself(remainingEffortForTaskItselfMillis, autoUpdateEffortEstimate, false);
    }
...
//    }
//</editor-fold>
    public void setRemainingForTaskItself(long remainingEffortForTaskItselfMillis) {
//        setRemaining(remainingEffortProjectTaskItselfMillis, true);
        setRemainingForTaskItself(remainingEffortForTaskItselfMillis, false); //false=> don't update the other field by default, only if explicitly defined
...
//        setRemainingForTask(getRemainingForTaskFromParse(), true);
        long remainingSubtasks = getRemainingForSubtasks();
        long remainingTotal = getRemainingTotal();
        long remainingTaskItself = getRemainingForTaskItself();

        //if task is (jjst) changed to project and subtasks have their own remaining, then check if task's own Remaining should be set to 0 or kept
        if (remainingSubtasks > 0 && remainingTaskItself > 0 && remainingTotal == remainingTaskItself) { //remainingTotal == remainingTaskItself should only be true first time we add a subtask with non-zero Remaining
            if (MyPrefs.keepRemainingForTaskItselfWhenATaskBecomesAProject.getBoolean()) {
                //do nothing, leave old Remaining for task itself even though subtasks now have their own Remaining
            } else {
                setRemainingForTaskItself(0, false);
//                setRemainingTotalInParse(remainingSubtasks, true);
                if (MyPrefs.addCommentWhenRemaningIsSetToZeroWhenTaskBecomesProject.getBoolean()) {
                    addToCommentDefaultPosition(Format.f("{0 Remaining} was changed from {1} to 0 since task was changed to projet", Item.EFFORT_REMAINING, "" + remainingTaskItself));
                }
            }
        }
...
//        return effort;
//    }
    public long getRemainingForTaskItself() {
//        return getRemainingForProjectTaskItself(true); //by default, 
        long effort = getRemainingForTaskItselfFromParse();
        return effort;
    }
//<editor-fold defaultstate="collapsed" desc="comment">
...
//    }
     ...
     */
//    @Override
//    public long getRemainingXXX() {
////        if (isDone()) {
////            return 0;
////        }
////        return getRemainingEffortFromParse();
////        return getRemaining(true, true);
//        return getRemaining(false, true); //default estimates are now always stored directly in Parse
//    }
//<editor-fold defaultstate="collapsed" desc="comment">
...
//    }
    private void setActualTotalInParse(long actualEffortTotalMillis, boolean updateStatus) {
...
//        long rounded = actualEffortTotalMillis; //no need to round, picker now handles millis - https://stackoverflow.com/questions/20385067/how-to-round-off-timestamp-in-milliseconds-to-nearest-seconds
        ...
        if (actualEffortTotalMillis != oldVal) {
            if (actualEffortTotalMillis > 0) {
                ...
                if (updateStatus && getStatus() == ItemStatus.ONGOING && oldVal > 0) { //TODO replace use of Settings by MyPrefs, only change status back from ongoing to created if an actual effort was already set
                    setStatus(ItemStatus.CREATED);
                }
            }
            updateEarnedValuePerHour();
...
//        }
//</editor-fold>
        if (false && autoUpdateStatusAndStartedOnDate) { //now done below in setActualTotalInParse (lowest level)
            if (actualEffortTotalMillis > 0) {
                //if setting Actual to a positive value (larger than zero) for the first time, and status==Created, set to Ongoing
//            if (getActualEffort() == 0 && getStatus() == ItemStatus.CREATED && Settings.getInstance().setStatusOngoingWhenActualEffortSetFirstTime()) {
//            if (autoUpdateStatusAndStartedOnDate && getStatus() == ItemStatus.CREATED && Settings.getInstance().setStatusOngoingWhenActualEffortSetFirstTime()) {
...
//    public boolean isActualAboveOngoingThresholdXXX(long totalActual) {
//        return getStatus() == ItemStatus.CREATED
//                && totalActual >= MyPrefs.timerMinimumTimeRequiredToSetTaskOngoingAndToUpdateActualsInSeconds.getInt() * MyDate.SECOND_IN_MILLISECONDS;
//    }
//    public boolean isActualAboveOngoingThresholdXXX() {
//        return isActualAboveOngoingThreshold(getActualTotal());
//    }
//<editor-fold defaultstate="collapsed" desc="comment">
...
//        }
//</editor-fold>
//        opsUpdateRepeatRule.add((owner) -> {
//            owner.updateCompletedOnSubtaskChange();
//            return true;
//        });
    }
...
//<editor-fold defaultstate="collapsed" desc="comment">
//    private void updateAggregatedValues() {
//        xxx;
//        updateAllValuesDerivedFromSubtasks(); //brute force
//
//        updateEstimateOnChangeInSubtasks(); //brute force
//        updateActualOnSubtaskChange(); //brute force
//        updateRemainingOnSubtaskChange(); //brute force
//
//        updateStartedOnDateOnSubtaskChange(); //brute force
//        updateCompletedOnSubtaskChange(); //brute force
//
//        updateStatusOnSubtaskStatusChange(); //brute force
//
//        for (String parseId : new ArrayList<String>()) {
//            switch (parseId) {
//                case PARSE_ACTUAL_EFFORT:
//                    updateActualOnSubtaskChange(); //brute force
//                    updateActualOnSubtaskChange(CLASS_PRIVATE);
//                    long actual = getActualForSubtasks();
//                    if (actual != getActual()) {
//                        setActual(actual, false);
//                    }
//                    break;
//            }
//        }
...
//        }
//</editor-fold>
            if (opsUpdateInheritedValues != null && opsUpdateInheritedValues.size() > 0) {
                List<Item> subtasks = getListFull();
                UpdateItem updateStatus = opsUpdateInheritedValues.remove(Item.STATUS);
                for (Item subtask : subtasks) {
                    boolean subtaskNeedsSaving = false;
//<editor-fold defaultstate="collapsed" desc="comment">
...
//</editor-fold>
                    ...
                    if (updateStatus != null) { //updateStatus may be null! See comments above
                        if (updateStatus.update(subtask)) {
                            subtaskNeedsSaving = true;
                        }
                    }
                    //then update other inherited fields
                    for (Object parseId : opsUpdateInheritedValues.keySet()) {
//<editor-fold defaultstate="collapsed" desc="comment">
//                    String parseId;
...
//</editor-fold>
                        UpdateItem updateSubtask = opsUpdateInheritedValues.get(parseId); //remove ensure each operation is only eexcuted once, even if same item is saved multiple times
                        if (updateSubtask.update(subtask)) {
                            subtaskNeedsSaving = true;
                        }
                    }
                    if (subtaskNeedsSaving) {
                        DAO.getInstance().saveNew(subtask);
                    }
                }
                opsUpdateInheritedValues.clear();
            }
        }
     ...
     * into its owner. Called after inserting new instances with
     * repeatRules**??. Also called when *changing* owner: already generated
     * repeat instances will remain with old owner, but new ones will be
     * generated in new owner.
     */
...
//        } else {
        ...
        if (currentRepeatRule != null && !isTemplate() && currentRepeatRule.isUpdatePending()) {
...
//    }
    @Override
    public void fireChangeEvent(ActionEvent evt) {
        List<Category> categories = getCategories();
        for (Category category : categories) {
            category.actionPerformed(evt);
        }
        ItemAndListCommonInterface.super.fireChangeEvent(evt);
    }
...
//        Item owner = getOwnerItem();
//        if (false && owner != null) {
//            for (UpdateItem update : opsUpdateRepeatRule) {
//                update.update(owner);
//            }
//        }
...
//        }
//</editor-fold>
        AlarmHandler.getInstance().update(this);
...
//        if (getDueDate().getTime() < MyDate.getStartOfToday().getTime()) { //NB. The length of the overdue time interval is decided when creating the list of tasks for Today
        ...
        } else if (MyDate.isToday(getWaitUntilDate())) {
            return TodaySortOrder.WAITING_TODAY;
        } else if (MyDate.isToday(getWaitingAlarmDate())) {
            return TodaySortOrder.WAITING_ALARM;
        } else if (MyDate.isToday(getAlarmDate())) {
            return TodaySortOrder.ALARM_TODAY;
        } else if (MyDate.isToday(getStartByDateD())) {
//<editor-fold defaultstate="collapsed" desc="comment">
//                waitingTime = getStartByDateD().getTime();
//                ASSERT.that(waitingTime >= startOfToday && waitingTime < startOfTomorrow, ()
...
//        return txt;
    ...
    String makeNotificationBodyText(AlarmType alarmType) {
        ...
        if (alarmType.isReminder()) {
            if (MyPrefs.alarmShowDueTimeAtEndOfNotificationText.getBoolean() && getDueDate().getTime() != 0) {
                s += (s.isEmpty() ? "" : "\n") + Item.DUE_DATE + ": " + MyDate.formatDateTimeNew(getDueDate());
            }
            if (MyPrefs.alarmShowAlarmTimeAtEndOfNotificationText.getBoolean() && getAlarmDate().getTime() != 0) {
                s += (s.isEmpty() ? "" : "\n") + Item.ALARM_DATE + ": " + MyDate.formatDateTimeNew(getAlarmDate());
            }
            if (MyPrefs.alarmShowSnoozeUntilTimeAtEndOfNotificationText.getBoolean() && getSnoozeAlarm().getTime() != 0) {
                s += (s.isEmpty() ? "" : "\n") + Item.SNOOZED_TILL + ": " + MyDate.formatDateTimeNew(getSnoozeAlarm().getTime());
            }
        } else if (alarmType.isWaitingReminder()) {
            ...
            if (MyPrefs.alarmShowSnoozeWaitingUntilTimeAtEndOfNotificationText.getBoolean() && getSnoozeWaitingAlarm().getTime() != 0) {
                s += (s.isEmpty() ? "" : "\n") + Item.SNOOZED_TILL + ": " + MyDate.formatDateTimeNew(getSnoozeWaitingAlarm().getTime());
            }
        }
//<editor-fold defaultstate="collapsed" desc="comment">
...
//        if (alarmType == AlarmType.snoozedNotif && MyPrefs.alarmShowSnoozeUntilTimeAtEndOfNotificationText.getBoolean()
//                && getSnoozeAlarm().getTime() != 0) {
//            s += (s.isEmpty() ? "" : "\n") + Item.SNOOZED_TILL + ": " + MyDate.formatDateTimeNew(getSnoozeAlarm().getTime());
//        }
//
//        if (alarmType == AlarmType.snoozedWaiting && MyPrefs.alarmShowSnoozeWaitingUntilTimeAtEndOfNotificationText.getBoolean()
//                && getSnoozeWaitingAlarm().getTime() != 0) {
//            s += (s.isEmpty() ? "" : "\n") + Item.SNOOZED_TILL + ": " + MyDate.formatDateTimeNew(getSnoozeWaitingAlarm().getTime());
//        }
//</editor-fold>
        return s;
...
//    private final static SimpleDateFormat csvDateTimeHhMmSsOnlyFormatter = new SimpleDateFormat(csvFormatStringDateHhMmSsOnly);
    ...
    final static String[] csvFieldList = new String[]{
        ...
        PARSE_URGENCY,
        PARSE_CATEGORIES,
        PARSE_EFFORT_ESTIMATE,
        PARSE_REMAINING_EFFORT_TOTAL,
        PARSE_ACTUAL_EFFORT,
        PARSE_EARNED_VALUE,
        PARSE_SUBTASKS,
        PARSE_DREAD_FUN_VALUE,
        PARSE_CHALLENGE,
        PARSE_EXPIRES_ON_DATE,
        PARSE_INTERRUPT_OR_INSTANT_TASK,
        PARSE_WAITING_ALARM_DATE,
        PARSE_REPEAT_RULE,
        PARSE_STARTED_ON_DATE,
        PARSE_HIDE_UNTIL_DATE,
        PARSE_START_BY_DATE,
        PARSE_INTERRUPTED_TASK,
        PARSE_DEPENDS_ON_TASK,
        PARSE_TEMPLATE,
        PARSE_SOURCE,
        PARSE_WAIT_UNTIL_DATE,
        PARSE_DATE_WHEN_SET_WAITING,
...
//<editor-fold defaultstate="collapsed" desc="the HUGE switch">
            switch (fieldId) {
                ...
                case PARSE_DREAD_FUN_VALUE:
                    if (toCSV) {
                        csv = csvNullString(getDreadFunValueN(), () -> getDreadFunValueN().getDescription());
                    } else {
                        setDreadFunValue(DreadFunValue.getValue(val));
                    }
                    break;
                case PARSE_CHALLENGE:
                    if (toCSV) {
                        csv = csvNullString(getChallengeN(), () -> getChallengeN().getDescription());
                    } else {
                        setChallenge(Challenge.getValue(val));
                    }
                    break;
                case PARSE_EXPIRES_ON_DATE:
                    if (toCSV) {
//                        csv=MyDate.formatDateNew(getExpiresOnDate()));
                        csv = csvDateToString(getExpiresOnDate(), csvDateFullFormatter);
                    } else {
                        setExpiresOnDate(csvStringToDate(val));
                    }
                    break;
                case PARSE_INTERRUPT_OR_INSTANT_TASK:
                    if (toCSV) {
//                        csv = ((Boolean) isInteruptOrInstantTask()).toString();
                        csv = csvBoolToString(isInteruptOrInstantTask());
                    } else {
                        setInteruptOrInstantTask(Boolean.parseBoolean(val));
                    }
                    break;
                case PARSE_ALARM_DATE:
                    if (toCSV) {
                        csv = csvDateToString(getAlarmDate(), csvDateFullFormatter);
                    } else {
                        setAlarmDate(csvStringToDate(val));
                    }
                    break;
                case PARSE_WAITING_ALARM_DATE:
                    if (toCSV) {
                        csv = csvDateToString(getWaitingAlarmDate(), csvDateFullFormatter);
                    } else {
                        setWaitingAlarmDate(csvStringToDate(val));
                    }
                    break;
                ...
                case PARSE_STARTED_ON_DATE:
                    if (toCSV) {
                        csv = csvDateToString(getStartedOnDateD(), csvDateFullFormatter);
                    } else {
                        setStartedOnDate(csvStringToDate(val), true); //force the explicit date (even if indirectly set via other field updates). Set dependent fields here? Could make sense if not set explicitly to ensure consistency, e.g. with status
                    }
                    break;
                case PARSE_DUE_DATE:
                    if (toCSV) {
                        csv = csvDateToString(getDueDate(), csvDateFullFormatter);
                    } else {
                        setDueDate(csvStringToDate(val));
                    }
                    break;
                case PARSE_HIDE_UNTIL_DATE:
                    if (toCSV) {
                        csv = csvDateToString(getHideUntilDateD(), csvDateFullFormatter);
                    } else {
                        setHideUntilDate(csvStringToDate(val));
                    }
                    break;
                case PARSE_START_BY_DATE:
                    if (toCSV) {
                        csv = csvDateToString(getStartByDateD(), csvDateFullFormatter);
                    } else {
                        setStartByDate(csvStringToDate(val));
                    }
                    break;
                case PARSE_WAIT_UNTIL_DATE:
                    if (toCSV) {
                        csv = csvDateToString(getWaitUntilDate(), csvDateFullFormatter);
                    } else {
                        setWaitUntilDate(csvStringToDate(val));
                    }
                    break;
                case PARSE_DATE_WHEN_SET_WAITING:
                    if (toCSV) {
                        csv = csvDateToString(getDateWhenSetWaiting(), csvDateFullFormatter);
                    } else {
                        setDateWhenSetWaiting(csvStringToDate(val));
                    }
                    break;
...
//                        csv = csvDurationToString(getActualForTaskItself(), csvDurationHhMmS);
                ...
                case PARSE_CATEGORIES:
                    ...
                    if (toCSV) {
                        csv = DAO.convCategoryListToString(getCategories());
                    } else {
                ...
                case PARSE_STARRED:
                    if (toCSV) {
//                        csv = ((Boolean) isStarred()).toString();
                        csv = csvBoolToString(isStarred());
                    } else {
                        setStarred(csvStringToBool(val));
                    }
                    break;
                ...
                case PARSE_COMPLETED_DATE:
                    if (toCSV) {
                        csv = csvDateToString(getCompletedDate(), csvDateFullFormatter);
                    } else {
                        setCompletedDate(csvStringToDate(val), true, true);
                    }
                    break;
                case PARSE_IMPORTANCE:
                    if (toCSV) {
                        csv = csvNullString(getImportanceN(), () -> getImportanceN().getDescription());
                    } else {
                ...
                case PARSE_URGENCY:
                    if (toCSV) {
                        csv = csvNullString(getUrgencyN(), () -> getUrgencyN().getDescription());
                    } else {
                ...
                case PARSE_OWNER_LIST:
                    ...
                    if (toCSV) {
                        csv = csvElementId(getOwnerItemList());
                    } else {
                        Object obj = csvElement(val);
                        if (obj instanceof ItemList) {
                            setOwnerItemList((ItemList) obj);
                        } else {
                            Log.p("Could not find owner List with ID = " + val);
                        }
                    }
                    break;
                case PARSE_OWNER_ITEM:
                    ...
                    if (toCSV) {
                        csv = csvElementId(getOwnerItem());
                    } else {
                        Object obj = csvElement(val);
                        if (obj instanceof Item) {
                            setOwnerItem((Item) obj);
                        } else {
                            Log.p("Could not find owner project with ID = " + val);
                        }
                    }
                    break;
                case PARSE_INTERRUPTED_TASK:
                    //TODO!!! search for task/project based on text string, if not existing, create it (?=> means need to check if already existing when importing/creating tasks) and add this task as sub-task
                    if (toCSV) {
                        csv = csvElementId(getTaskInterrupted());
                    } else {
                        Object obj = csvElement(val);
                        if (obj instanceof Item) {
                            setTaskInterrupted((Item) obj);
                        } else {
                            Log.p("Could not find interrupted task with ID = " + val);
                        }
                    }
                    break;
                case PARSE_DEPENDS_ON_TASK:
                    //TODO!!! search for task/project based on text string, if not existing, create it (?=> means need to check if already existing when importing/creating tasks) and add this task as sub-task
                    if (toCSV) {
                        csv = csvElementId(getDependingOnTask());
                    } else {
                        Object obj = csvElement(val);
                        if (obj instanceof Item) {
                            setDependingOnTask((Item) obj);
                        } else {
                            Log.p("Could not find depending on task with ID = " + val);
                        }
                    }
                    break;
                case PARSE_TEMPLATE:
                    if (toCSV) {
//                        csv = ((Boolean) isTemplate()).toString();
                        csv = csvBoolToString(isTemplate());
                    } else {
                        if (false) { //if importaing Templates, template is automatically set, so no use for it here
                            boolean isTemplate = csvStringToBool(val);
...
//                        csv = MyDate.formatDateNew(getUpdatedAt());
                ...
                case PARSE_SOURCE:
                    //TODO!!! search for task/project based on text string, if not existing, create it (?=> means need to check if already existing when importing/creating tasks) and add this task as sub-task
                    if (toCSV) {
                        csv = csvElementId(getSource());
                    } else {
                        Object obj = csvElement(val);
                        if (obj instanceof Item) {
                            setSource((Item) obj);
                        } else {
                            Log.p("Could not find source task with ID = " + val);
                        }
                    }
                    break;
...
//                requiredCalc += getRemainingForProjectTaskItself(false); //false=> no default effort (for a project with subtasks, only subtasks should have default values)
                requiredCalc += getRemainingForTaskItself(); //false=> no default effort (for a project with subtasks, only subtasks should have default values)
            } else { //leaf task
...
//    }
    /**
     * will return MyDate.MAX_DATE if any subtask does not have a finishTime,
     * otherwise the latest of all the finishTimes
     *
     * @return
     */
    private long getLatestSubtaskFinishTime() {
        long latestFinishTime = MyDate.MIN_DATE; //return MIN_DATE if no later finishTime is found
        boolean someSubtasksHaveNoFinishTime = false;
        Item subtask;
...
//                    if (finishT > latestFinishTime && !subtask.isDone()) {
                if (finishT == MyDate.MAX_DATE) {
                    someSubtasksHaveNoFinishTime = true;
                } else if (finishT > latestFinishTime) {
                    latestFinishTime = finishT;
                }
            }
        }
        return someSubtasksHaveNoFinishTime ? MyDate.MAX_DATE : latestFinishTime;
    }
...
//            }
//</editor-fold>
            long latestSubtaskFinishTime = getLatestSubtaskFinishTime(); //super.getFinishTime() is the project task's own finishTime in case it has its own Remaining
            ...
            if (latestSubtaskFinishTime != MyDate.MAX_DATE && projTaskFinishTime != MyDate.MAX_DATE) {
                if (projTaskFinishTime > latestSubtaskFinishTime) {
                    latestSubtaskFinishTime = projTaskFinishTime;
                }
            }
...
//            return latestFinishTime == MyDate.MIN_DATE ? MyDate.MAX_DATE : latestFinishTime;
            return latestSubtaskFinishTime;
//            }
        } else {
...
//    }
    public String hasReferencesToUnsavedParseObjects() {
        ...
        if (getRepeatRuleN() != null && getRepeatRuleN().getObjectIdP() == null) {
            str += " | No ObjId for RepeatRule:" + getRepeatRuleN();
        }
...
//            boolean addAsSubtasks = false;
            ...
            int indentLevel = countIndentChars(line, "-\t ");
    ...
    public String getDebugTextInfo(boolean getInfo) {
        ...
        if (getInfo) {
            return (item.getObjectIdP() == null ? "-OBJID" : "")
                    ...
                    + (item.getWorkSlotListN() != null && item.getWorkSlotListN().size() > 0 ? "[W]" : "")
                    + (TimerStack2.getInstance().getCurrentlyTimedItemN() == item ? " [Timed]" : "")
                    + (item.isTemplate() ? "%" : "")
                    //                + ((Config.TEST && MyPrefs.showDebugInfoInLabelsEtc.getBoolean()) && item.getSource() != null && item.getSource().isTemplate() ? "/T" : "")
                    + (item.getSource() != null && item.getSource().isTemplate() ? "[fr Templ]" : "")
                    //if showing Item
                    //                + (item.getOwner() != null && !(item.getOwner().equals(orgList)) ? " /[" + item.getOwner().getText() + "]" : ""
                    + (item.getOwner() != null && item.getOwner() instanceof Item ? "^" : "");
        } else {
            return "";
        }
    }
...
//        return new Date[]{getDueDate(), getStartByDateD(), getAlarmDate(), getWaitUntilDate(), getWaitingAlarmDate()};
        return new Date[]{getDueDate(), getStartByDateD(), getAlarmDate(), getWaitUntilDate(), getWaitingAlarmDate(), getSnoozeAlarm(), getSnoozeWaitingAlarm()};
    }

    /**
     * returns the next date where a task need attention (is due, has an alarm,
     * is waiting until, has waiting reminder, should start). Use to
     * list/include tasks in calendar view (should be combined with for example
     * showing the given date in bold in the task view).
     *
     * @return return the first coming date
     */
    public Date getNextDate() {
        Date nextComingDate = new MyDate(MyDate.MAX_DATE);
//        Date[] dates = new Date[]{getDueDate(), getStartByDateD(), getAlarmDate(), getWaitUntilDate(), getWaitingAlarmDate()};
        Date[] dates = getTodayDates();
        for (Date date : dates) {
            if (date != null && date.getTime() != 0 && date.getTime() < nextComingDate.getTime()) {
                nextComingDate = date;
            }
        }
