...
//    final static String PARSE_DELETED_DATE = "deletedDate"; //has this object been deleted on some device?
    ...
    public boolean hasUserModifiedData() {
        //OWNER not user-modifiable
        //SOURCE not user-modifiable
        //DURATION and END_TIME are both equivalent
        boolean userModifiedData
                = isDirty(PARSE_TEXT)
                || isDirty(PARSE_REPEAT_RULE)
                || isDirty(PARSE_START_TIME)
                || isDirty(PARSE_DURATION)
                || isDirty(PARSE_END_TIME);
...
//    public WorkSlot(String description, Date start, int durationInMinutes, RepeatRuleParseObject myRepeatRule) {//, boolean importedFromPIM) {
    public WorkSlot(boolean setEditedDate) {//, boolean importedFromPIM) {
        this();
        if (setEditedDate) {
    ...
    public WorkSlot(String description, Date start, long durationInMillis, RepeatRuleParseObject myRepeatRule, boolean setEditedDate) {//, boolean importedFromPIM) {
        this();
        if (setEditedDate) {
            setEditedDateToNow(); //UI: always make the creation of a task an 'edit' event
        }
    ...
    public WorkSlot(String description, Date start, long durationInMillis, RepeatRuleParseObject myRepeatRule) {//, boolean importedFromPIM) {
        this(description, start, durationInMillis, myRepeatRule, true);
    }
...
//</editor-fold>
    ...
    public WorkSlot(WorkSlot sourceWorkSlot, Date startDate) {
        this(sourceWorkSlot.getText(), startDate, sourceWorkSlot.getDurationInMinutes(), sourceWorkSlot.getRepeatRuleN());
...
//        boolean differentOwner = MyUtil.neql(newOwnerItem, oldOwner);
        ItemList oldOwner = getOwnerItemList();
        boolean change = !Objects.equals(ownerList, oldOwner);
        if (change) {
            if (ownerList != null) {
                put(PARSE_OWNER_LIST, ownerList);
            } else {
...
//        }
        ItemList oldVal = getOwnerCategory();
        boolean change = !Objects.equals(category, oldVal);
        if (change) {
            if (category != null) {
                put(PARSE_OWNER_CATEGORY, category);
            } else {
...
//        }
        Item oldVal = getOwnerItem();
        boolean change = !Objects.equals(ownerItem, oldVal);
        if (change) {
            if (ownerItem != null) {
                put(PARSE_OWNER_ITEM, ownerItem);
            } else {
...
//</editor-fold>
        if (oldRepeatRule == null) { //setting a RR for the first time
            ...
            //newRepeatRule != null and possibly modified (eg. click Edit Rule, then Back
...
//        return new WorkSlot(this, referenceTime);
    ...
    public WorkSlot cloneMe(CopyMode copyFieldDefinition) {
        WorkSlot newCopy = new WorkSlot(true);
    ...
    void copyMeInto(WorkSlot destination, CopyMode copyFieldDefinition) {
        ...
        if (copyFieldDefinition == CopyMode.COPY_TO_REPEAT_INSTANCE) {
            ...
            destination.setRepeatRule(getRepeatRuleN());
        }
    }
...
//        }
//</editor-fold>
        Date oldVal = getStartTimeD();
        boolean change = !Objects.equals(start, oldVal);
        if (change) {
...
//</editor-fold>
    ...
    private final void setEndTime(Date endTime) {
        Date oldVal = getEndTimeD();
        boolean change = !Objects.equals(endTime, oldVal);
        if (change) {
...
//        }
    ...
    /**
     * workslot duration is always set by the user (no automatic calculation
     * that could millisecond results) so only store minutes in Parse
     *
     * @return
     */
...
//</editor-fold>
    ...
    public long getDurationAdjusted(long fromTime, long toTime) {
        if (Config.TEST) {
            ...
            Date actualEnd = new MyDate(Math.min(getStartTimeD().getTime() + getDurationInMillis(), toTime)); //get the earliest endDate
            Date actualStart = new MyDate(Math.max(getStartTimeD().getTime(), fromTime)); //get the latest startDate
...
//        );
    ...
    public boolean hasActiveDuration(long now) {
...
//        setDurationInMillis(durationInMinutes * MINUTES_IN_MILLISECONDS);
        int oldVal = getDurationInMinutes();
        boolean change = !Objects.equals(durationInMinutes, oldVal);
        if (change) {
...
//</editor-fold>
    ...
    public boolean hasSaveableData() {
        return (getStartTimeD().getTime() != 0 && getDurationInMinutes() != 0); //TODO! should also check for new/changed repeat rule
    }
...
//    }
    /**
     * keep track of which items have been allocated a (non-zero?!) slide of
     * this workslot
     *
     * @param item
     */
...
//    }
    ...
    public void onDelete(Date deletedDate) {
        ...
        //DELETE IN OWNER
//<editor-fold defaultstate="collapsed" desc="comment">
//        if (false && owner != null) { //done in removeFromOwner()
//            ItemAndListCommonInterface owner = getOwner();
//            WorkSlotList workSlotList = owner.getWorkSlotListN();
...
//        return sortedWorkslotList;
    ...
    public void setEditedDate(Date editedDate) {
        ...
        boolean change = !Objects.equals(editedDate, oldVal);
        if (change) {
    ...
    /**
     * return start/end dates if they fall within today, or if the workslot
     * overlaps with today, return start and/or end of today
     */
